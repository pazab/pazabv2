...
> ⚠️ **2026-08-11부로 이 문서 무효화됨.** 이 문서는 "가입 동선 밖으로 빼고 CTA로 유도"하는 절충안이었는데,
> 콜드스타트 단계에서 성향분석이 대타 SOS 루프(최우선 기능, `lib/daetaTier.ts`)엔 전혀 안 쓰이고
> 정규 채용 매칭 스코어(`/api/match`)에만 40% 가중치로 기여하는데 후보가 적어 체감가치가 낮은 반면
> 인터뷰마다 Claude API 비용이 실비로 나간다고 판단해서, **CTA/배너/뱃지/카드 전부 제거 + 매칭 가중치 제거**로
> 더 나아갔다. 인터뷰/분석 백엔드(`/api/interview`, `/api/analyze`)와 기존 사용자 데이터는 안 지웠고,
> 코드도 삭제 안 했음 — 진입 링크만 전부 끊은 상태(PAZ 챗과 동일 패턴). 상세는
> `docs/20260811_PAZAB_DEV_HANDOVER.md` 참조. 나중에 밀도 생기면 이 문서 다시 참고해서 재활성화 가능.
