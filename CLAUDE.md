...
> 상세 전략·우선순위·신기능 체크리스트는 **[STRATEGY.md](STRATEGY.md)** 참조. 아래는 핵심 요약.

파잡의 1순위 문제는 가입 마찰이 아니라 **빈 마켓플레이스**. 아래 원칙은 온보딩 UI가 바뀌어도 유효한 상위 전략:

- **한 줄 정의**: "가게 관리 앱인데, 펑크 나면 버튼 하나로 검증된 대타가 오는 앱"
...
- **대타 SOS 루프가 최우선 코딩 목표**: 펑크 발생→요청→수락→자동계약/정산 원스톱. 데모 가능한 상태 만들기가 현재 1순위.
- **2-Tier 대타 풀**: Tier1(✅검증 — 팀이력/근태이력 있음) 선노출(첫 10~30분), Tier2(🔵신규) 하단. 한번 일하면 Tier1 승격. 시급 강제 하한 금지 — 긴급 할증 기본값 넛지로 대신.
- **피드·팔로우·탐색 확장 동결**: 코드 삭제 X, 네비 강등. 동네 밀도 생긴 후 재개.
- **계좌인증 비대칭**: 사장님은 SOS 전 필수, 알바생은 정산 확정 순간 1회만.
- **초대 공급 경로**: 콜드 획득보다 "이미 일하는 사람"을 카톡 초대로 편입 → 마찰 최소, 즉시 검증 상태.
- **HEXACO는 백엔드 신호**: 노쇼 예측·매칭 랭킹용. 사장님에겐 "노쇼 위험 낮음" 배지 하나만 노출. 가입 동선 밖.
- **집중 지역**: 탕정/아산 한 상권에서 먼저 밀도 증명 후 확장.
