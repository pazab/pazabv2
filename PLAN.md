# PLAN.md
> 최종 업데이트: 2026-07-29 | PAZAB v2 (`C:\pazabv2`, Supabase: clrjxxkgceluvzvrkvyl)

## 구현 완료

**"파잡 커리어"(`/worker/[id]`) 역할 무관 통합 커리어 페이지로 확장 + 구직 갤러리 사진 독립 관리 (2026-07-29 2차)**
- 전날 계획(아래 "다음 세션 작업" 항목) 실행 완료. `worker_profiles`는 라이브 DB에 unique 제약이 없지만 실사용 전원 1개씩이라 코드 레벨에서 유저당 1개로 확정 유지.
- **`app/worker/[id]/page.tsx` 헤더 분기**: `worker_profiles` 있으면 기존 히어로 배너(사진 캐러셀+매칭배지) 그대로, 없으면 `users.avatar_url` 기반 컴팩트 헤더(아바타+닉네임+등급)로 표시 — 순수 사장님도 텅 빈 카드 대신 정체성만 담백하게 나옴.
- **"구직 정보" 섹션 조건부화**: 핵심조건/자격증/자기소개/성향/AI봇/궁합/팀궁합/하단 매칭 액션바(러브콜·SOS·수락거절) 전부 `worker_profiles` 있을 때만 표시 — 매칭 상태머신 로직 자체는 안 건드림. 없으면(본인 화면만) "💼 구직 정보를 아직 등록 안 하셨어요" 유도 카드.
- **"🏪 운영 매장" 섹션 신규**: `/profile/[userId]`에만 있던 것을 이식(대상 유저의 `employer_profiles` 조회), `/profile/[userId]`쪽 중복은 제거(관련 쿼리·state도 정리).
- **구직 갤러리(최대 10장+영상) 독립 관리 — 여러 번 방향 수정 후 최종본**:
  1. 1차 시도: "사진만 바꾸기"(`users.avatar_url` 원형 크롭 직접 수정)를 만들었는데, 구직정보 있으면 `worker_profiles.image_url`에도 동기화해버려서 "사진만 바꾸기"랑 "구직카드 사진 갤러리" 둘 다 같은 사진이 바뀌는 것처럼 보이는 문제 발생.
  2. 사용자 피드백으로 재검토: 프로필 사진(`avatar_url`) 수정은 이미 마이페이지에 있어서 파잡커리어에 또 만들 필요가 없다고 판단 — **"사진만 바꾸기" 기능 자체를 완전히 제거**(state/핸들러/크롭모달까지 삭제).
  3. "🖼️ 구직카드 사진" 메뉴만 남기고, `hasWorkerProfile` 조건 없이 항상 노출하도록 변경 — **구직 정보(희망직종/지역 등)를 하나도 안 써도 사진만 먼저 등록 가능**. `app/worker/profile/page.tsx`에 `start=gallery` 진입 시 프로필 유무와 상관없이 1단계(필수 입력) 건너뛰고 바로 사진 화면으로 이동, `handleSaveGalleryOnly()` 신규(기존 있으면 update, 없으면 `user_id`만 채운 최소 row로 insert) — 사진만 등록한 유저는 나머지 항목이 "협의"로 표시되는 정상적인 degrade.
- **`worker_profiles` 중복 생성 잠재 버그 제거**: `app/explore/page.tsx`(글쓰기 FAB)·`app/mypage/page.tsx`("+ 구직 공고" 버튼) 둘 다 이미 프로필 있으면 `new=true` 없이 편집으로만 이동하도록 분기. `worker/profile/page.tsx`의 `handleSave`도 `existing` 있으면 `new=true` 여부와 무관하게 항상 update하도록 수정(전엔 이미 프로필 있는 사람이 "새로 쓰기" 진입하면 insert 시도해서 중복 row 생길 위험 있었음), 이제 다 안 쓰는 컴포넌트 레벨 `isNew` 변수도 제거.
- 전 항목 `npx tsc --noEmit` 신규 에러 0건 확인 (기존 무관 에러 2개 파일은 그대로).

**프로필 라우팅 전면 정리 + 스키마 드리프트(paz_*, job_likes) 대규모 발견/수정 (2026-07-29)**
- **대타 SOS 지원 오차단 버그**: `app/api/lovecall/route.ts`의 "이미 소속된 팀원" 체크가 매장 구분 없이 사장님 단위(`team_members`엔 store 컬럼 자체가 없음)로만 걸려 있어서, 다른 매장 소속 팀원이 대타 SOS에 지원하면 차단당함 — `lib/daetaEscalation.ts`의 `notifyTeam()`이 애초에 그 팀원들한테 지원하라고 알림을 보내는데 정작 지원을 막는 자기모순이었음. `daetaPostingId` 있을 때는 이 체크를 스킵하도록 수정, "진행중 매칭 있으면 차단" 체크도 같은 공고/대타 건끼리만 비교하도록 스코프 좁힘.
- **프로필 링크 통일 규칙 확정 및 전면 적용**: 워커는 항상 `/worker/[id]`(구직카드), 사장님은 매장 맥락 있으면 `/store/[id]` 없으면 `/profile/[id]`(정체성 허브)로 통일. 채팅 헤더(아바타+이름 따로 가던 버그, 사장님 상대는 `users.id`를 `/job/[id]`에 잘못 넘겨서 항상 깨져있던 것도 같이 해결 — `goToCounterpartProfile()` 헬퍼로 통합), 마이팀 팀원 바텀시트("전체 프로필 보기" 탈출구 추가), 탐색 후보카드 바텀시트, 마이페이지 러브콜 4개 섹션(받은/보낸 지원·제안 전부 소셜프로필 대신 매칭 상대 상세로) 전부 이 규칙으로 정리.
- **`app/employer/[id]/page.tsx` 삭제**: 앱 어디서도 링크가 없는 고아 페이지였고, 조회하는 `employer_profiles.rating/review_count`·`reviews` 테이블이 스키마에 아예 없어 도달해도 항상 빈 값만 나오는 죽은 코드.
- **`/worker/[id]`(파잡 커리어) 콘텐츠 재검토**: STRATEGY.md("HEXACO는 백엔드 신호, 사장님에겐 '노쇼 위험 낮음' 배지 하나만 노출")와 실제 구현이 어긋나 있던 것 확인 — HEXACO 6개 축 그래프(`HexacoSection`) 통째로 제거, `users.worker_result.noShowRisk === "낮음"`을 `TierBadge`의 기존 `noShowSafe` prop(원래 있었는데 안 쓰이던 기능)에 연결해 배지 하나로 대체. "최근 활동" 피드 미리보기(동결 대상 기능이 구직카드에 섞여 있던 것) 제거. `/profile/[userId]`의 "구직 정보" 섹션도 `/worker/[id]`와 완전 중복이라 제거.
- **`app/job/[id]/page.tsx`의 `isOwner` 버그**: state는 선언돼 있는데 `setIsOwner(true)`를 호출하는 코드가 파일 어디에도 없어서, 사장님이 본인 공고를 봐도 "수정하기" 대신 "신고하기/차단하기"가 뜨고 본인 공고에 "지원하기" 버튼까지 뜨던 버그. `fetchJob`에서 `uid === data.user_id` 체크 추가해서 수정.
- **`worker_type` localStorage 오염 문제**: `app/worker/profile/page.tsx`의 `handleSave`가 매 저장마다(사진만 바꿔도) 브라우저 localStorage의 성향검사 결과를 조건 없이 반영해서, 성향검사 안 한 걸로 아는 유저 화면에 계속 성향 카드가 뜨거나 반대로 다른 기기에서 저장하면 기존 값이 null로 지워지는 양방향 문제. "로컬에 값 있을 때만 반영"으로 수정(없으면 필드 자체를 안 보내 기존 DB값 보존). pazab@kakao.com의 기존 `worker_profiles.worker_type`도 서비스롤 스크립트로 초기화.
- **"사진만 바꾸기" 빠른 진입 경로**: `/worker/[id]` "⋯" 메뉴를 "📸 사진만 바꾸기"(`start=media`로 1단계 필수입력 건너뛰고 바로 미디어 업로드)와 "📝 구직 조건 전체 수정"으로 분리. 크롭 종횡비가 `aspect={1} isCircle={true}`(원형, 아바타용 설정 오적용)였던 것을 라벨이 이미 명시하던 "권장 3:2 비율"에 맞게 `aspect={3/2}`로 수정. 저장 후 무조건 `/explore?type=worker`로 튕기던 것(예전 "잡보드" 시절 잔재)도 진입 시 `return` 파라미터를 넘기도록 고쳐서 원래 보던 화면으로 복귀하게 수정 — 그 과정에서 `return`이 "/"로 시작하기만 하면 기존 프로필 로딩을 통째로 건너뛰고 신규 등록 취급하던 조건문 버그도 같이 발견해 제거(`app/explore/page.tsx`의 기존 "수정" 플로우도 같은 버그를 갖고 있었어서 동시에 고쳐짐).
- **PAZ 스키마 드리프트 대규모 발견/수정** — 서비스롤 키로 라이브 DB 직접 조회해서 `supabase/v2_schema.sql`이 실제 스키마와 크게 어긋나 있음을 확인(예: `employer_profiles.user_id`가 문서엔 UNIQUE인데 실제로 오리옹 계정이 매장 3개 보유 — 문서가 구버전). `users.paz_avatar/paz_photo_url/paz_theme/paz_name/paz_knowledge/paz_style/paz_speech/paz_auto_speak/paz_last_report_at` 컬럼이 전부 라이브 DB에 없음(42703 에러) 확정.
  - **가장 심각했던 것**: `app/paz/page.tsx`의 프로필 로딩 쿼리가 존재하는 컬럼(`nickname/worker_result/user_type`)과 없는 컬럼(`paz_name` 등)을 한 select에 같이 넣어서, PostgREST가 통째로 실패 → **PAZ AI 채팅 페이지를 열면 인사말도 대화이력도 성향 컨텍스트도 아무것도 안 불러와지고 있었음.** 정상 컬럼만 select하도록 분리해서 실제로 복구.
  - "주간 감정 리포트"는 마지막 발송 시각 컬럼(`paz_last_report_at`)이 없어 항상 "7일 지남" 조건이 참이 돼 메시지마다 스팸처럼 뜨고 있었음 — 제거. "20개마다 AI로 대화 요약해서 장기기억 저장"도 저장은 항상 실패하면서 AI Gate 호출(비용)만 매번 발생하고 있었음 — 제거. `app/paz/settings/page.tsx`(전체가 깨진 컬럼 전용 설정 화면)와 3곳의 진입 링크(`app/settings`, `PazFloatingButton` 메뉴, paz 페이지 헤더) 전부 제거. `PazFloatingButton.tsx`/`FloatingChatWidget.tsx`의 아바타/테마 조회도 깨진 컬럼 빼고 기본값(🤖, 퍼플)으로 하드코딩 — 단, `user_type`(정상 컬럼, 음성 인텐트 라우팅에 실사용)은 살려서 별도 조회로 분리.
- **`job_likes` 테이블 자체가 스키마에 없음 확인**(PostgREST가 "혹시 feed_likes 아니냐"고 되물음 — 그건 있음): `worker/[id]`·`job/[id]`의 좋아요(♡) 버튼, `/api/likes`(존재하지도 않는 라우트) 호출, `api/match/route.ts`의 `job_likes` 조회 전부 제거. **미완료**: `app/explore/page.tsx`가 같은 깨진 `/api/likes`를 쓰는데 좋아요 버튼이 공용 컴포넌트 `components/JobCard.tsx`에 있어서 영향범위가 더 넓어 이번엔 보류.
- **다음 세션 작업 (계획 확정, 코드는 미착수)**: "파잡 커리어"(`/worker/[id]`)를 역할 무관 통합 커리어 페이지로 확장 — 헤더는 `users.avatar_url` 기반으로 항상 표시(구직정보 있을 때만 지금의 히어로 배너로 업그레이드), "구직 정보"는 `worker_profiles` 있을 때만 조건부 표시(없으면 유도 카드), "운영 매장" 섹션을 `/profile/[userId]`에서 이식해 `employer_profiles` 있으면 표시. `worker_profiles`는 라이브 DB엔 unique 제약이 없지만(문서만 그렇게 돼있음) 실사용 4명 전원 1개씩이라 코드 레벨에서 유저당 1개로 확정 — `explore.tsx`/`mypage.tsx`의 "새 프로필 만들기" 플로우(중복 insert 에러 위험 있던 잠재 버그) 제거 예정. "사진만 바꾸기"는 `users.avatar_url` 원형 크롭 직접 수정으로 재정의(워커 위저드 경유 안 함). 전체 계획은 `~/.claude/plans/flickering-foraging-planet.md` 참조.
- 전 항목 `npx tsc --noEmit` 신규 에러 0건 확인.

**방향성 재점검(STRATEGY.md §11) + 성장 지표 어드민 신설 + admin 계정 이관 + MY 페이지 정리 (2026-07-28)**
- **STRATEGY.md §11 신규 — 대화를 통해 정리한 방향성 재확정**: (11.1) "대타"는 사람 자격이 아니라 상황(응급 시프트) 정의이고 Tier1/Tier2는 별개 축인 신뢰 레이어라는 점 명시, `allow_new` opt-in 게이트가 이미 이 원칙을 구현 중임을 확인. (11.2) 신규 기능·마케팅 판단 우선순위를 "사장님이 등록/초대한 기존 직원 > 대타(Tier2)로 유입된 신규 > 완전 콜드 신규 가입자"로 명문화. (11.3) 하단 네비 피드 탭 위치 — 2026-07-13 강등 결정이 2026-07-27 5탭 개편에서 근거 기록 없이 되돌아간 것을 확인했으나, 실제로는 소셜 프로필(`/profile/[userId]`) 기능이 의미를 가지려면 피드 접근 동선이 필요하다는 근거 있는 판단이었음을 확인 → **피드 탭 유지로 재확정**(단, 밀도 낮은 지역 빈 피드 리스크는 계속 관찰, 필요시 기본 필터를 팔로잉/내활동으로 좁히는 안 검토). (11.4) §2 핵심 성장지표("사장님 1명 가입 → 직원 N명 Tier1 편입")를 실제로 재는 곳이 없다는 계측 공백 확인, 다음 최우선 작업으로 지정.
- **§4 승격조건에 구현 갭 경고 추가**: `lib/daetaTier.ts`가 사장님 평가 없이 매칭 성사 여부만으로 Tier1 승격시키는 문제 — "검증됨"이 잡보드 자기주장 수준과 다르지 않게 되는 리스크. 미착수, 다음 작업 후보로 기록.
- **`/admin/growth-stats` 신규** (`app/admin/growth-stats/page.tsx`, `app/api/admin/growth-stats/route.ts`): §11.4 계측 공백을 메우는 첫 페이지. 최근 7일 신규 등록/초대 팀원 수, 현재 대타가능(`available_now`) 켠 인원, 대타 SOS 발생/성사 건수·성사율을 서비스롤 집계로 표시. 기존 `/admin/ai-stats`와 동일한 클라이언트 이메일 게이트 패턴 사용.
- **admin 계정 전체 이관 (`hellopazab@gmail.com` → `pazab@kakao.com`)**: v2 실사용 메인 계정(카카오)이 admin 권한도 갖도록 `app/admin/ai-stats/page.tsx`, `app/admin/tax-rates/page.tsx`, `app/admin/trust/page.tsx`, `app/admin/growth-stats/page.tsx`, `app/api/admin/growth-stats/route.ts`의 `ADMIN_EMAIL` 상수 전부 교체. `CLAUDE.md` 테스트 계정 목록도 갱신(`hellopazab@gmail.com`은 구 admin으로 표기만 남김). 약관/개인정보/VAPID push 연락처의 `hellopazab@gmail.com`은 권한 체크가 아니라 법적 고지·기술 규격용이라 그대로 유지.
- **MY 페이지(`app/mypage/page.tsx`) 정리**:
  - "지원 현황"/"채용 제안" 카드에 `daeta_posting_id` 유무로 `🚨 대타에서 시작` 배지 추가 — 대타 SOS에서 전환된 매칭과 일반 지원을 구분 가능하게.
  - "파잡 커리어"/"소셜 프로필" 버튼에 아이콘(💼/👤) 추가하고 "내 팀·소속"/"팀원 초대"와 동일한 카드 레이아웃(아이콘 위→굵은 제목→회색 부제)으로 통일, 감싸고 있던 "내 공개 프로필 카드 미리보기" 래퍼 박스 제거하고 4개 버튼 크기·그리드를 완전히 통일.
  - `LoveCallSection` 접힌 상태 기본 노출 건수를 3건 → 1건으로 축소(정렬 로직은 그대로: 진행중 우선, 동순위는 최신순).
- 전 항목 `npx tsc --noEmit` 신규 에러 0건 확인(기존 무관 에러 `app/explore/page.tsx`, `app/job/[id]/page.tsx` 2개 파일은 그대로 둠).

**근태(출퇴근) 시스템 버그 다수 수정 + 팀원 개인정보 조회/수정 기능 신규, 매장 스택카드 디자인 확정 (2026-07-26 2차)**
- **근태 중복 레코드 버그 — "오늘 출근 3/2"의 원인**: `attendance(team_member_id, work_date)`에 유니크 제약이 없는 상태로 알바생의 `handleCheckIn()`이 매번 plain INSERT를 해서, 크론이 만든 결근 행 위에 또 새 행이 쌓이는 중복이 발생. 홈 화면 "오늘 출근 X/Y" 배지의 분자(X)가 분모(Y)보다 커지는 형태로 드러남. `handleCheckIn()`을 upsert(`onConflict: team_member_id,work_date`)로 전환하고, `supabase/patch_attendance_unique.sql` 작성해 기존 중복 정리 + 유니크 제약 추가(사용자가 Supabase SQL Editor에서 직접 실행, 완료 확인함).
- **근무요일 매칭 로직 3중 불일치 — 결근 자동처리가 조용히 빠지던 원인**: `app/api/cron/checkin`(20분 미출근 자동 결근 처리), `app/myteam/page.tsx`(시간표), `app/employer/team/[id]/page.tsx`(예정일 캘린더 표시)가 각자 다른 로직으로 `work_days`를 파싱했고, 셋 다 "평일"/"주말" 매크로 표현을 아예 인식 못 했음(이 매크로 표현으로 저장된 팀원은 크론의 결근 대상 목록에서 통째로 빠짐 — 크론 자체는 5분마다 정상 실행 중이었는데도 특정 팀원한테 도달을 안 한 것). `lib/utils.ts`에 `isWorkingOnDay()` 단일 구현으로 통일(개별 요일 나열 + 평일/주말/주5일/주6일/매일 매크로 전부 지원), 크론·마이팀·팀원상세 세 곳 모두 이 함수로 교체.
- **출퇴근 버튼 시간/요일 게이팅 신규**: 계약 근무 종료시각이 지나면 "출근하기" 버튼이 비활성화되고 "사장님께 확인 요청" 안내로 전환(퇴근 버튼은 초과근무 등 정상 상황 고려해 그대로 유지). 오늘이 정규 근무요일이 아닌데 출근을 누르면 alert 대신 바텀시트 확인("대타/추가근무면 계속 진행")을 거치도록 추가 — 실수로 엉뚱한 날 체크인되는 것 방지.
- **팀원 상세 근태 캘린더 — 계약 근무시간 기본값 자동 채움**: 날짜 클릭 시 기존 기록 없으면(결근/휴무 제외) 계약서상 근무시간을 시작/종료 select에 미리 채워서 "정상"은 저장 버튼 한 번, 지각/조퇴도 필요한 부분만 수정하면 되게 개선(30분 단위 select라 계약 시간은 가까운 30분으로 반올림).
- **팀원 상세 "👤 개인정보" 신규 (생년월일/연락처/주소)**: `users` 프로필과 계약서 스냅샷(`contract_data.workerPhone` 등) 중 있는 쪽으로 자동 병합해 표시, 수정 시 둘 다 정정(계약서 조회 화면도 오탈자가 안 남게). 기본은 마스킹(`010-****-5678`, "만 27세", 주소 앞 2토큰)이고 "👁 보기"로 펼쳐봄.
  - **RLS 이슈 발견 및 수정**: `users` 테이블 UPDATE 정책이 본인 전용(`auth.uid() = id`)이라 사장님이 클라이언트에서 직접 못 고치는데도 에러 없이 조용히 0건 업데이트되던 버그. `app/api/team/personal-info/route.ts` 신규 — 세션에서 실제 로그인 유저를 서버가 직접 검증해 "이 팀원의 진짜 사장님"일 때만 서비스 롤로 처리하도록 전환.
  - 알바생 계약서 동의 화면(`app/chat/[id]/page.tsx`)의 연락처 입력에도 형식 검증 추가 — 처음엔 `needPhone`(프로필에 번호가 아예 없을 때)에만 검증이 걸려 있어서, 프로필에 이미 번호가 있는 사람이 계약서용으로 새 번호를 잘못 입력해도 그냥 저장되던 버그를 같이 수정.
- **매장 스택카드 디자인 — 여러 번 반복 후 최종본 확정**: 버튼탭 방식·아코디언 방식 시도했다가 전부 반려(겹친 카드 방식을 유지하고 싶어함), 최종적으로 방향A 이전(2026-07 초) 오리지널 그라데이션 팔레트를 git 히스토리에서 복원하되 색상환에서 서로 확실히 떨어진 8색으로 교체. "오늘 출근" 표시를 단순 카운트에서 "출근/예정" 분수(`X/Y`)로 변경.
- **다음에 확인할 것**: 이번 수정은 전부 "앞으로"만 적용됨 — 이미 발생한 과거 결근 누락은 소급 적용 안 되니 달력에서 수동 입력 필요.

**사장님 홈 UX 재검토 — 타이포 스케일·레이아웃 재설계 시도 후 전부 되돌림, "홈/관리 분리" 계획으로 대체 (2026-07-26)**
- **시도 1 — 타이포·터치 스케일 상향(`DESIGN_PLAN.md` §10)**: KWCAG 2.2·실측 대비비 기준으로 폰트 11~15px→16px+, 터치 타겟 48px 이상 목표 확정 후 67개 파일에 걸쳐 적용(서브에이전트 병렬 처리, 세션 한도로 여러 번 끊김 → 우선순위 재조정하며 완주, `npx tsc --noEmit` 신규 에러 0건 검증 완료).
- **시도 2 — 사장님 홈(`/myteam`) 팀원 목록을 예외우선 체크리스트로 재구성**: 지각·결근·미출근만 펼치고 정상 출근자는 접는 방식(안 B) + 한 줄 요약 배너(안 A) 구현, 이후 페이지 최상단으로 재배치, 온보딩 완료 시 나머지 섹션 자동 접힘까지 추가.
- **문제**: 두 시도 모두 사용자에게 "그대로인 것 같다"는 피드백을 받음 — 실제 원인은 "접어서 숨기기"로는 화면에 여전히 존재한다는 인지 부담 자체가 안 줄어든다는 것(Jitterbug/Lively, Apple Design Award 사례 재조사로 확인). 리사이즈·재배치가 아니라 **페이지를 물리적으로 분리**해야 한다는 결론.
- **최종 처리**: 사용자 요청으로 `git reset --hard backup/pre-typography-scale-2026-07-25`(커밋 `726f98f`, §10 계획 문서만 남긴 지점)로 전체 되돌림. 되돌린 커밋들(타이포 스케일 60여개 + 레이아웃 재설계)은 워크트리 브랜치 `claude/uiux-design-direction-6e8d23`와 위 태그에 그대로 남아있어 필요 시 부분 재적용 가능.
- **다음 세션 작업**: [HOME_SPLIT_PLAN.md](HOME_SPLIT_PLAN.md) 참조 — `/myteam`을 "오늘 확인"(출근 현황+계약확인 배지)만 남기는 최소 홈으로 줄이고, 매장 관리 전체(매장 정보·팀원 전체·시간표·초대)를 현재 고아 페이지인 `/employer/team`으로 이전. 5단계로 순서·검증 방법까지 확정된 상태, 코드는 아직 미착수.

**디자인 토큰 확산 완료, 뒤로가기 전면 점검, 매장 데이터 정합성 버그 다수 수정 (2026-07-24)**
- **디자인 토큰 확산 마무리**: `/chat`, `/chat/[id]`, `/mypage`, `/explore`, `/job/[id]`, `/worker/[id]` 남은 화면 전부 방향 A(플랫 미니멀) 토큰·아이콘 규칙 적용 (자세한 건 `DESIGN_PLAN.md` §9 4차 참조). `DaetaRegisterModal.tsx`는 라이트모드에서 텍스트가 아예 안 보이던 버그(모달 전체 `color:"#fff"` 하드코딩)를 발견해 전체 토큰화, 매장 선택도 드롭다운 → 버튼 칩으로 변경.
- **뒤로가기 전면 감사 (`app/contract/page.tsx`, `app/worker/profile/page.tsx`, `app/employer/register/page.tsx`, `app/payslip/page.tsx`)**:
  - 저장 성공 후 `router.push`를 쓰던 곳들(계약서/구직정보/공고 등록/급여명세서 발행)이 전부 `router.replace`로 교체돼, 저장 직후 뒤로가기 시 방금 제출한 폼으로 되돌아가던 문제 해결.
  - `app/contract/page.tsx`: 저장 후 이동 분기(from=chat/memberId/from=team) 중 아무 것도 안 걸리는 무컨텍스트 진입 시 아예 이동을 안 해서 폼에 그대로 남던 케이스에 기본 이동(`/mypage`) 추가.
  - `app/payslip/page.tsx`: 뒤로가기가 실제 진입 경로와 무관하게 항상 `/myteam`으로 하드코딩돼 있던 것 제거, 기본 브라우저 히스토리 back으로 정상화.
- **매장 데이터 정합성 버그 2건 — `employer_profiles.is_active`를 "매장이 살아있는지"로 오용**: `is_active`는 실제로는 "일반 공고를 올렸는지" 플래그(`StoreRegisterModal.tsx`에서 신규 매장은 `is_active:false`로 생성)인데, `DaetaRegisterModal.tsx`의 매장 목록 조회와 `app/daeta/page.tsx`의 SOS 발송(`handleCall`)이 이 값으로 필터링해서 공고를 한 번도 안 올린 매장이 조용히 빠지던 버그. 두 곳 다 `is_deleted` 기준으로 교체. **이 버그가 실제로 대타 등록 시 "매장 없음" 오판 → 매번 재등록 → 매장 중복 생성의 근본 원인이었음** (사장님 1명 계정에 같은 매장 이름으로 5개 중복 발견, SQL로 정리).
  - 대타 게시글 저장 시 `daeta_postings.required_credentials` 컬럼이 아예 없어 저장 실패하던 것도 확인 — `patch_missing_columns.sql`이 이 컬럼을 `employer_profiles`에만 추가하고 `daeta_postings`엔 빠뜨렸던 이관 실수. `supabase/patch_daeta_required_credentials.sql`로 보완 (실행 완료).
- **team_members.employer_profile_id 누락 — 매장 여러 개인 사장님 계약서에서 엉뚱한 매장 뜨는 버그**: 채팅에서 직접 채용확정(`hire_accept`)할 때와 계약서 서명 API 양쪽 다 `matches.employer_profile_id`를 넘기지 않고 team_members를 생성해서, `/contract`가 "가장 최근 등록한 매장"으로 잘못 폴백. `app/api/chat/route.ts`가 애초에 그 컬럼을 select 안 하던 게 원인 — select에 추가하고 두 생성 경로 다 실제 값을 넘기도록 수정.
- **`/contract` 페이지 자체의 불일치 버그**: "매장 선택 칩"과 "주소/사업자정보 폼"을 서로 다른 독립 쿼리로 채우고 있어서, 매장이 여러 개면 두 쿼리의 "최근 매장" 폴백이 서로 다른 걸 골라 화면 안에서 매장이 안 맞는 프랑켄슈타인 현상 발생. `loadByMember`/`load`/`handleSelectMatch` 세 곳 다 매장 정보를 항상 매장칩 목록(`availableEps`/`myEps`)에서 찾아 쓰도록 통일. `load()`의 폴백 쿼리에 존재하지 않는 컬럼명(`biz_address`)이 섞여 있던 것도 같이 제거.
- **근로자 개인정보(생년월일/연락처/주소) 수집 시점 변경**: 원래 계약서 "근로자" 스텝에서 필수였는데, 사장님이 정확히 모를 수 있는 정보라 여기선 선택으로 풀고(`app/contract/page.tsx`), 대신 알바생이 "계약서 동의" 하는 순간(`app/chat/[id]/page.tsx`의 `showSignConfirm`)에 비어있는 항목만 인라인으로 채우게 하고 `users`에 저장하도록 이동. 이 화면에 토스트(`useToast`)도 새로 연결.
- **서류함 (등본/보건증/통장사본) — 스키마만 작성, 아직 미실행/미구현**: `supabase/patch_team_documents.sql`에 `team_member_documents` 테이블 정의 완료 (team_member_id, doc_type: resident_registration/health_certificate/bank_copy, file_url, expires_at, uploaded_by + RLS). **Supabase에 아직 실행 안 함.** 파일은 기존 `media` 스토리지 버킷 재사용 예정, 새 버킷 불필요. 계약서는 이미 `contracts` 테이블이 있어서 이 테이블 대상에서 제외.
  - **다음에 할 일**: (1) 위 SQL Supabase에서 실행 (2) 팀원 상세(`app/employer/team/[id]/page.tsx`) 페이지에 서류함 섹션 UI 추가 — 문서 타입별 업로드 버튼 + 미리보기 + 삭제 (3) 업로드는 클라이언트에서 `media` 버킷에 직접 올리고 `team_member_documents`에 메타데이터만 insert (4) 보건증은 만료일 입력 필드 추가하고, 가능하면 만료 임박 알림(기존 알림 시스템 재사용) 연동
  - 주민등록번호는 구조화된 필드로 저장하지 않고(개인정보보호법 이슈), 4대보험 신고 등 실제 필요 시점에만 다루는 방향으로 합의, 별도 구현 없음.

**매칭 프로세스 고도화, 양방향 알림 및 탐색 페이지 매칭 상태 연동 (2026-07-18)**
- **양방향 알림 발송 연동 (`app/api/lovecall/route.ts`)**:
  - 지원서 발송 또는 채용 제안(러브콜) 성공 시 송신자와 수신자 양쪽 모두에게 각각 알림(푸시 및 앱 내 알림)이 적재 및 발송되도록 `createNotification` 로직 추가.
- **첫 지원/제안 시 확인 모달 전면 적용**:
  - 거절/취소된 이력이 있을 때 뿐만 아니라 첫 지원/제안 클릭 시에도 "지원 확인/채용 제안 확인" 커스텀 모달을 띄워 사용자 실수 방지.
- **지원 완료 후 1:1 채팅방 이동 제안 및 정정**:
  - **[기류 수정]** 지원/제안은 수락 대기(pending) 상태이므로 즉각적인 대화방 입장이 아닌, 상대방이 수락했을 때 대화방이 개설되는 것이 타당합니다. 따라서 지원/제안 즉시 노출되던 "채팅방으로 이동" 모달을 단순 성공 알림("확인" 버튼)으로 간소화 개편했습니다.
- **탐색 페이지 내 매칭 상태 뱃지 및 바텀시트 CTA 동적 제어 및 즉시 지원/제안 기능 (`app/explore/page.tsx`, `components/JobCard.tsx`)**:
  - 탐색 페이지 로딩 시 사용자의 실시간 매칭 상태 리스트를 fetch하여 state로 관리.
  - `JobCard`에 `matchStatus`/`matchIsSent`를 전달하여 카드 위에 `📤 수락 대기`, `💬 대화 중`, `✅ 채용 완료` 뱃지를 Glassmorphism 스타일로 노출.
  - 탐색 카드 탭 시 나오는 상세 팝업 바텀시트의 [지원하기] 버튼도 매칭 상태(수락 대기중 비활성화, 대화 중 -> `💬 대화 중 · 채팅방 가기`로 이동)에 따라 동적으로 반응하도록 전면 개편.
  - **[UX 개선]** 바텀시트 상세 팝업에서 [지원하기] / [제안하기]를 눌렀을 때, 굳이 상세 페이지로 리다이렉트 하지 않고 **그 자리에서 확인 모달을 띄우고 즉시 지원 처리**하도록 변경. 지원이 성공하면 실시간으로 카드 뱃지가 갱신되며 "지원 완료" 단순 알림 모달 노출 완료.
- **수락 대기중(pending) 매칭의 채팅 목록 차단 및 마이페이지 기본 전개 설정 (`app/api/chatrooms/route.ts`, `app/mypage/page.tsx`)**:
  - `pending` 상태의 매칭은 아직 매칭이 승인된 것이 아니므로 `/chat` 채팅방 목록 API에서 걸러지도록 쿼리를 원상복귀(`accepted`, `interviewing`, `hired`만 노출).
  - 마이페이지 내 구직활동(지원 현황) 및 채용활동(채용 제안) 아코디언이 내역이 있을 시 접혀서 안 보이는 현상을 방지하기 위해 기본 전개 상태(`true`)로 수정.
- **[채팅 꼬임 해결] 신규 매칭 생성 시 시스템 메시지 전송 및 중복 정렬/수락 배너 연동 (`app/api/lovecall/route.ts`, `app/api/chatrooms/route.ts`, `app/chat/[id]/page.tsx`, `app/api/chat/route.ts`)**:
  - 지원/제안 전송 시 `chats` 테이블에 즉각 시스템 메시지(`📤 지원서 전송` / `💌 채용 제안 전송`)를 추가하여, 신규 매칭이 즉시 대화방 목록 최상단에 뜨도록 수정.
  - 대화방 목록 조회 시 동일 유저 조합 of 이전 매칭(과거 이력)이 먼저 선택되어 노출이 씹히는 문제를 방지하고자, 중복 제거 전에 `created_at` 내림차순으로 우선 정렬하도록 변경.
  - 1:1 대화방(`/chat/[id]`) 상단에 `pending` 상태일 때의 수락/거절 액션 배너를 추가하여, 마이페이지에 갈 필요 없이 대화방 안에서 바로 매칭 수락 및 거절 처리가 가능하도록 전면 개선.
  - **[기능 보완]** `/api/chat` GET API에서 `initiated_by` 필드를 추가로 select하도록 보완하여, 수락 대기중 배너가 발송자에게는 `"수락 대기중..."`으로, 수신자에게만 `"수락하기/거절하기"`로 올바르게 표시되도록 정정했습니다.
  - **[모순 메시지 제거]** 개별 대화방 방식 적용으로 더 이상 과거 내역이 병합되지 않으므로, 새 매칭 수락 시 떴던 `"이전 대화에 이어서 진행해요"` 시스템 메시지를 제거했습니다.
  - **[기능 보완]** 상대방이 채팅창을 나갔을 때(`worker_left` / `employer_left` 상태가 true가 되었을 때) 상단에 `"상대방이 채팅방을 나갔습니다"`라는 명확한 탈퇴 알림 배너가 렌더링되도록 구현했습니다. Matches 상태 폴링 쿼리에도 해당 컬럼들을 추가하여 실시간 감지가 가능합니다.
  - **[기능 보완]** 알바생이 지원을 취소하거나 사장님이 제안을 취소(`action === "cancel"`)할 시, 상대방에게 실시간 알림(`🚫 지원 취소 / 제안 취소`)을 발송하여 취소 사실을 알리도록 구현했습니다.
- **[하이브리드 UX 구현] 상세 이력서/공고 프로필 상세 페이지 내 즉시 수락/거절 추가 및 알림 연동 (`app/job/[id]/page.tsx`, `app/worker/[id]/page.tsx`, `app/api/lovecall/route.ts`, `lib/notify.ts`)**:
  - 알바생 이력서 상세(`/worker/[id]`) 및 매장 공고 상세(`/job/[id]`) 페이지의 하단 바에 수락 대기(pending) 상태의 제안서/지원서가 들어와 있을 경우, 기존 비활성 버튼 대신 실제 **`[수락하기]` / `[거절하기]` 버튼이 뜨도록 동적으로 교체**했습니다.
  - 해당 상세화면에서 버튼 클릭 시 즉시 PATCH API 호출과 함께 매칭 성사 시스템 메시지가 인서트되고, 마이페이지와 동일한 **미려한 "매칭 성사!" 완료 팝업(AI 사전미팅 하기 / 바로 채팅하기 / 나중에)이 노출**되도록 하여 매끄러운 1:1 대화 진입을 지원합니다.
  - 지원 도착 알림의 랜딩 URL을 기존 `/mypage`에서 **수신자의 상세 프로필(`/worker/[id]` 또는 `/job/[id]`)로 직접 연결**되도록 갱신하여 사장님이 지원서를 검토하고 그 자리에서 원클릭으로 매칭 수락 및 채팅을 시작할 수 있게 만들었습니다.
  - **[버그 수정]** 알림 발송 모듈(`lib/notify.ts`)에서 알림 생성 시 전달받은 `url` 속성을 DB의 `data` 필드 오브젝트에 병합하도록 수정하여, 인앱 알림 목록에서 터치 시 기획했던 상세 랜딩 화면으로 정상 이동하게 조치했습니다.
  - **[기능 보완]** 이전 대화방 이력을 들고 오지 않고 나가기 후 새로 지원 시 대화방이 새롭게 리셋되도록 `/api/chat` 메시지 쿼리를 `matchId` 기준으로 엄격히 제한(개별 대화방 방식 적용)하였습니다.
  - **[UX 개선] 대화방 내 상단 '채용 프로세스 액션 가이드 배너' 전면 배치 (`app/chat/[id]/page.tsx`)**:
    - 면접 예약이나 채용 제안 기능이 우측 상단 `···` 메뉴 속에 숨겨져 있어 사용자가 채용 단계를 전혀 인지하지 못하던 치명적인 결함을 극복했습니다.
    - 이제 대화방 상단에 사장님/알바생 상태에 따른 동적 가이드 바를 직관적으로 상시 노출합니다:
      - **대화중 (`accepted`)**: 사장님 상단에 **`[📅 면접 예약]` 및 `[✅ 채용 제안]`** 버튼 노출.
      - **면접예약중 (`interviewing`)**:
        - **면접 진행 단계**: 사장님에게 **`[일정 수정]`, `[바로 채용]`** 퀵 액션과 함께 면접이 끝난 뒤 피드백할 수 있는 **`[✅ 면접 완료]`, `[❌ 면접 취소]`, `[🚫 노쇼 신고]`** 대시보드 직접 노출.
        - **채용 제안 수락 대기 단계**: 사장님에게는 `"채용 제안 수락 대기 중"` 상태를, 알바생에게는 상단에 **`[수락하기] / [거절하기]`** 선택 바를 직접 제공하여, 채팅 중 한눈에 다음 단계를 조작할 수 있도록 흐름을 가이드합니다.
  - **[채용 최종 거절 프로세스 고도화 - 대안 A 적용] 채용 거절 시 매칭 실패 처리 및 대화 차단 (`app/chat/[id]/page.tsx`, `app/api/lovecall/route.ts`)**:
    - 알바생이 사장님의 최종 채용 제안을 거절(Rejection)할 시, 실수 방지를 위해 **커스텀 디자인된 전용 컨펌 모달(⚠️ 경고 테마 디자인)**을 띄워 거절 여부를 한 번 더 확인받습니다.
    - 수락 거절 확정 시, 서버 PATCH API(`action: "hire_reject"`)를 호출하여 해당 매칭 상태를 `failed`로 변경하고, 공고 및 알바생 구직 정보는 자동으로 다시 지원 가능 상태(`active`)로 복귀시킵니다.
    - 대화방 내부에는 `"💔 알바생이 채용 제안을 거절하여 매칭이 종료되었습니다."` 시스템 메시지를 인서트하고, 사장님에게 실시간 거절 알림을 발송합니다.
    - 매칭이 결렬되었으므로 대화방 하단 **텍스트/음성 입력 영역을 즉시 차단(비활성화)하고 `"종료된 매칭으로 메시지를 보낼 수 없습니다."` 경고 바를 노출**하여 불필요한 대화 지속을 방지하였습니다.
  - **[이력 기반 경고 강화] 실패/취소/거절 이력 대상자 재지원/재제안 시 확인 단계 추가 (`app/job/[id]/page.tsx`, `app/worker/[id]/page.tsx`, `app/explore/page.tsx`)**:
    - 이전에 매칭이 거절(`rejected`), 취소(`cancelled`), 또는 실패(`failed`)된 이력이 있는 상대방에게 다시 지원이나 제안을 보낼 때, 그냥 묻지 않고 **"이전에 매칭이 결렬(거절/취소/실패)된 이력이 있는 대상입니다. 다시 지원/제안하시겠습니까?"**라는 별도의 이력 기반 컨펌 모달을 노출하여 사용자 실수를 방지하고 과거 이력을 인지할 수 있도록 보완했습니다.
    - 특히 **탐색 메인 페이지(`app/explore/page.tsx`)**에서 바로 지원서/제안서를 다시 보낼 때도 동일하게 이력 감지 및 컨펌 단계를 거치도록 예외 없이 통합 연동했습니다.
    - 상세 페이지 하단 퀵 액션 버튼 역시 이전 매칭이 결렬(`failed`, `cancelled`)되었을 때 standard 버튼이 아닌 **`"💔 결렬됨 · 다시 보내기"`** 특수 상태 버튼으로 올바르게 렌더링되게 분기를 추가(재지원 플로우 일치)했습니다.
  - **[버그 수정] 매칭 조회 시 최신 날짜 순(descending) 정렬 누락으로 인한 상태 불일치 수정 (`app/explore/page.tsx`, `app/api/chatrooms/route.ts`)**:
    - 탐색 메인(`app/explore/page.tsx`)에서 `matches` 목록을 조회할 때 `created_at` 기준 내림차순 정렬이 누락되어, 동일한 대상과의 과거 매칭 이력(예: `hired` 완료)이 최신 매칭 이력(예: `pending` 대기중, `failed` 결렬)보다 우선 감지되는 오작동을 수정했습니다.
    - 이제 탐색 페이지 내의 모든 Supabase `matches` 조회 쿼리에 `.order("created_at", { ascending: false })` 정렬 조건을 추가하여 최신 매칭 건이 우선 대조되도록 수정했습니다.
    - 또한 채팅방 목록 조회 API (`app/api/chatrooms/route.ts`)에서도 중복 제거(deduplicate) 처리 이전에 매칭 목록을 최신순으로 정렬하는 로직을 보충하여 항상 가장 최근에 생성된 유효한 매칭 채팅방이 나타나도록 보완했습니다.
  - **[버그 수정] 마이페이지 내 복수 미디어 업로드 게시글(스토리) 썸네일 및 상세 뷰 렌더링 정상화 (`app/mypage/page.tsx`)**:
    - 이미지와 비디오가 섞여서 업로드된 게시물의 전체 `media_type`이 `"video"`로 설정되었을 때, 썸네일 그리드가 첫 번째 미디어가 이미지인데도 `<video>` 태그에 URL을 넣어 엑박이 뜨던 치명적 오동작을 수정했습니다.
    - 이제 개별 미디어 URL 확장자를 감지하여 올바른 태그(`<video>` vs `<img>`)로 랜딩하고, 비디오의 경우 `#t=0.1` 및 메타데이터/음소거 설정을 부여하여 첫 프레임이 정상 렌더링되게 조치했습니다.
    - 라이트박스(상세보기) 내에서도 `media_type` 분기 대신 개별 미디어 타입을 각각 감지하여 스크롤 캐러셀 형태로 비디오와 이미지가 공존하며 정상 노출되도록 개선했습니다.
    - 다중 미디어의 경우 썸네일 우측 상단에 겹친 카드 형태의 배지(`ti-layers-difference`)를 노출하는 UI 디테일을 보완했습니다.

**구직자 상세·매장 홈·프로필 페이지 UI 통합 개선 (2026-07-16)**
- **`/worker/[id]` 전면 재설계** (`app/worker/[id]/page.tsx`):
  - `/job/[id]` 공고 상세 페이지를 기준으로 레이아웃·스타일 통일
  - 히어로: 미디어 캐러셀 + 배지 + 지역 + 프로필 칩 버튼(이름+아바타) → `/profile/[id]` 연결
  - 2×2 조건 그리드(희망 시급/근무요일/근무시간/경력), 보유 자격증·실무기술 배지 섹션
  - **📸 최근 활동** 섹션 추가: `feed_posts` 테이블에서 해당 유저의 게시물 최신 2개를 매장 소식과 동일한 스타일로 표시
  - 하단 CTA: 본인 → "구직정보 수정하기", 사장님/타인 → "채용 제안하기"
  - 화면 너비 flex shrink-to-fit 버그 수정: `width: "100%", maxWidth: 480, margin: "0 auto"` 적용
- **`/store/[id]` 신뢰도 보기 버튼 → 프로필 칩으로 교체** (`app/store/[id]/page.tsx`):
  - 기존 텍스트 링크 "신뢰도 보기 →" 대신 아바타+이름 칩 형태로 표시
  - 클릭 시 사장님 `/profile/[user_id]` 페이지 이동
- **`/profile/[userId]` 너비 고정** (`app/profile/[userId]/page.tsx`):
  - `width: "100%", maxWidth: 480, margin: "0 auto"` 추가 → 다른 페이지와 동일한 480px 중앙 컬럼으로 통일
- **flex-column body shrink-to-fit 함정 정리**:
  - `layout.tsx`의 `body { flex flex-col }`에서 자식 `<main>`에 `margin: 0 auto`만 주면 콘텐츠 크기로 축소됨
  - 전 페이지 공통 해결책: `width: "100%"` 필수 병기

**미디어 캐러셀 동적 비디오 판별 및 구직자 자격증 스텝 재배치 (2026-07-15)**
- **동영상/사진 복합 미디어 캐러셀 고도화**:
  - 피드 등에서 여러 장의 미디어를 등록/조회할 때 동영상(mp4 등)과 사진이 혼재될 경우 레이아웃이 깨지던 문제 해결.
  - 미디어 확장자를 동적으로 판별하여 `video` 태그와 `img` 태그로 분기 처리함으로써, 단일/다중 미디어 캐러셀 슬라이더 내에서 비디오와 이미지가 깨짐 없이 원활하게 구동되도록 정돈.
- **구직자 프로필 보유 자격증 영역 상향 배치**:
  - 구직 프로필 등록 화면(`app/worker/profile/page.tsx`) 내에서 기존 2단계 최하단에 떨어져 있던 `보유 자격증 및 실무기술` 입력 영역을 1단계의 `💼 희망 직종` 바로 하단이자 `📍 희망 지역` 바로 위로 상향 이식 완료.
  - 직종/직무를 기입하는 흐름에 맞춰 자격증 보유 항목을 즉각 이어서 확인할 수 있도록 UX 스텝 레이아웃 조정.

**통합 유저 프로필 페이지 고도화 (2026-07-13 6차)**
- **진입점 단일화**: 채팅방 헤더 아바타·메시지 버블 아바타 클릭 → 기존 `UserProfileBottomSheet` 바텀시트 제거, `/profile/[userId]` 풀페이지 직행 (`app/chat/[id]/page.tsx`)
- **`⋯` 액션 메뉴** (`app/profile/[userId]/page.tsx`): 헤더 우상단, 본인 프로필엔 미표시. 💬 1:1 메시지 보내기(`/api/dm` 경유) / 🚫 차단하기·해제(`user_blocks` 테이블)
- **신뢰 검증 섹션**: 신뢰도 게이지바 + 역할별 배지 분기
  - 알바생: 🛡️신원인증 / 🏦계좌검증 / 🔬성향분석 / 🤝매칭완료 (4개)
  - 사장님: 🛡️신원인증 / 🔬성향분석 / 🤝매칭완료 (3개, 계좌검증·보건증 제외)
  - 매칭완료: `matches` hired/accepted/completed 기록 존재 여부로 판정
  - 선호 업종 칩 알바생만 표시
- **피드 라이트박스 풀스크린 전환**: 미니 모달 → 인스타 스타일 전체화면(다중이미지 좌우 화살표·인디케이터, 댓글 인라인, 배경 탭 닫기)
- `UserProfileBottomSheet` 파일 보존 (myteam에서 계속 사용)

**`/profile/[userId]` 라이트/다크 테마 완전 대응 (2026-07-13 5차, 5차-2 정정)**
- **1차 시도(폐기)**: 배경이 고정 다크 그라디언트(`#181528→#09090b`)인데 텍스트가 테마 변수(`var(--text)`)라 라이트모드에서 검정 글씨가 검정 배경에 얹혀 안 보이던 문제 → 처음엔 "텍스트를 고정 밝은색으로" 접근했으나, 사용자 피드백으로 "왜 이 페이지만 다크냐, 앱 전체와 통일감 있게 라이트/다크 다 따라가야" 방향으로 정정
- **최종 수정**: 배경을 `var(--bg)`로, 카드 배경/보더를 전부 `var(--surface)`/`var(--card-inner)`/`var(--border)`/`var(--card-inner-border)`로, 뱃지 색은 `var(--chip-purple-bg)`/`var(--chip-pink-bg)`/`var(--purple-text)`/`var(--pink-text)` 등 이미 globals.css에 있던 라이트·다크 겸용 토큰으로 전면 교체. 앱 나머지 페이지(mypage/feed/store)와 동일한 테마 대응 원칙 적용 완료

**개인 프로필 vs 매장 홈 역할 분리 + 프로필 페이지 삭제 매장 노출 버그 수정 (2026-07-13 4차)**
- **문제**: `/profile/[userId]`가 `employer_profiles` 조회 시 `is_deleted` 필터가 빠져있어 삭제된/중복 테스트 매장까지 전부 카드로 노출(실사용자 기준 8개 중 5개가 삭제된 매장). 다른 페이지들은 예전에 이 필터가 추가됐는데 프로필 페이지만 누락됐던 케이스
- **수정**: `.or("is_deleted.is.null,is_deleted.eq.false")` 필터 추가 + `business_name` null 제외
- **역할 재정의**: 개인 프로필(`/profile/[userId]`)과 매장 홈(`/store/[id]`)이 매장 상세정보(업종/위치/공고)를 중복 표시하면서 사용자가 "어디로 가야 하는지" 헷갈리는 문제 발견 → 프로필 페이지의 "사장님 정보" 섹션을 매장명+칩(채용중 여부)만 있는 목록으로 단순화하고 클릭 시 무조건 `/store/[id]`로 연결. 매장 상세(업종/위치/공고/소식/팔로우)는 매장 홈에만 존재하도록 역할 분리. 피드 카드 작성자 클릭 규칙(매장 글→매장 홈, 개인 글→개인 프로필)은 기존 그대로 유지

**피드 UI 폴리싱 — 색상 버그 근본원인 수정 + 인스타식 미디어 인터랙션 (2026-07-13 3차)**
- **근본원인 발견**: Tailwind v4는 `@theme` 블록 안에 `--color-*` 접두사로 선언된 변수만 `bg-primary`/`text-text-sub`/`border-border` 같은 유틸리티 클래스로 컴파일한다. `app/globals.css`는 `--primary`, `--text-sub` 등을 `@theme` 밖 `:root`에 접두사 없이 선언해놔서, 앱 전역(`/feed`, `/mypage`, `/profile/[userId]`, `/store/[id]`)에서 이 패턴을 쓴 버튼·카드가 전부 무효 클래스로 처리되어 배경/글자색이 안 먹고 투명하게 보이던 문제였음. `@theme inline`에 `--color-primary: var(--primary)` 등 매핑 추가로 근본 수정 (`app/globals.css`)
- **`app/feed/page.tsx` 등록 버튼 등 핵심 CTA 인라인 스타일 전환**: 위 수정과 별개로, 캐시/재시작 이슈에도 영향 안 받도록 FAB·필터탭·매장선택 chip·모달 취소/공유하기·댓글 전송 버튼을 `var(--primary)` 인라인 스타일로 확정
- **다중 이미지 캐러셀 PC 대응**: 스크롤 제스처만 있던 다중 이미지에 좌우 화살표 버튼 추가, "1/N" 인디케이터가 실제 스크롤 위치를 반영하도록 수정 (`mediaIndexMap`/`mediaRefs`)
- **인스타식 풀스크린 이미지 뷰어**: 이미지 탭 또는 우하단 🔍 아이콘 클릭 시 전체화면 라이트박스(캡션·작성자·좋아요/댓글, 좌우 넘기기, 배경클릭/X로 닫기) 오픈. 좋아요는 카드와 실시간 동기화(`liveZoomedPost`)
- **동영상 스크롤 자동재생/정지**: `IntersectionObserver`로 화면에 60% 이상 노출되면 자동재생(음소거 기본), 벗어나면 정지. 우하단 🔇 버튼으로 음소거 토글, 영상 탭으로 수동 재생/정지 가능

**하단 네비 채팅 복귀 + 피드 발견 경로 보강 (2026-07-13 2차)**
- **문제의식**: 채팅(매칭→계약→근무로 이어지는 핵심 전환 경로)이 하단 네비에서 빠지고 플로팅 버튼으로 격하돼 있었음. 반면 신규 매장 홈/팔로우 기능은 피드에 새 글이 올라올 때 우연히 스쳐야만 발견 가능해 "지속적으로 보러 온다"는 원래 의도가 완성되지 않았음
- **`components/BottomNav.tsx`**: 하단 네비 탭 `피드` → `채팅`으로 교체(`/chat`, 안읽음 뱃지 연결). 피드는 탭에서 내려가고 `AppHeader` ⋮메뉴(📰 피드)로 이동
- **`app/layout.tsx`**: `FloatingChatWidget` 전역 마운트 제거 — 채팅 진입점이 플로팅버튼/전체보기/마이페이지 카드 3곳으로 중복되던 것을 하단 탭 1곳으로 정리 (컴포넌트 파일 자체는 미사용 상태로 보존, 삭제는 보류)
- **`components/AppHeader.tsx`**: ⋮ 드롭다운에 "📰 피드" 메뉴 항목 추가 (대타SOS/스도쿠/설정과 같은 자리)
- **`app/feed/page.tsx`**: 필터 탭에 "❤️ 팔로잉" 추가 — `store_follows`로 내가 팔로우한 매장 글만 모아보기 가능. 팔로잉 탭 빈 상태일 땐 "전체 피드 둘러보기" 유도 문구로 분기
- **`app/job/[id]/page.tsx`**: 공고 상세에 "🏪 매장 홈 보기" 버튼 추가 (`job.employer_profile_id` → `/store/[id]`) — 탐색/공고 열람 중에도 매장 홈으로 유입되도록 발견 경로 보강

**매장 홈(스토어 페이지) + 피드-매장 귀속 + 팔로우 기능 신규 구현 (2026-07-13)**
- **문제의식**: 공고는 마감되면 사라지는 일회성 콘텐츠라 사장님이 지속적으로 홍보할 채널이 없었음. 반대로 피드는 있었지만 매장과 무관한 개인 SNS처럼 동작해 채용 목적과 분리돼 있었음.
- **DB**: `supabase/patch_feed_store.sql` — `feed_posts.employer_profile_id` 컬럼 추가(다중 매장 사장님이 특정 매장에 글을 귀속시킬 수 있도록), `store_follows` 테이블 신규(유저가 매장을 팔로우) + RLS
- **피드 작성 흐름 개선** (`app/feed/page.tsx`): 사장님/both 유저는 작성 모달에서 "개인 소식" vs 보유 매장 중 하나를 선택해 글을 올릴 수 있음(기본값: 첫 매장). 매장에 올린 글은 카드에 "매장홈 →" 뱃지가 붙고 작성자 영역 클릭 시 `/store/[id]`로 이동(개인 글은 `/profile/[userId]`로 이동)
- **`/api/feed` GET/POST 개선** (`app/api/feed/route.ts`): 매장 귀속 글은 `employer_profile_id` 기준으로 해당 매장의 이름·이미지를 정확히 매핑(기존엔 다중 매장 사장님의 경우 `user_id`로만 조회해 엉뚱한 매장 정보가 섞일 수 있었음). POST 시 본인 소유 매장인지 검증 후 저장, 저장 성공 시 해당 매장 팔로워 전원에게 `feed_new_post` 알림 발송(자연 재방문 유도)
- **`/api/store/follow` 신규**: 매장 팔로우/언팔로우 토글, 신규 팔로우 시 매장 사장님에게 `store_follow` 알림
- **`app/store/[id]/page.tsx` 신규 — 매장 홈 페이지**: 매장 커버 이미지·업종·설명, 팔로우 버튼+팔로워 수, 진행중인 공고 고정 카드(`jobs` 테이블에서 `is_active` 기준 조회) + 해당 매장 피드 3열 그리드(라이트박스 상세에서 좋아요/댓글). 즉 "공고는 왔다가 사라져도, 매장 페이지 자체는 영구 URL로 남아 재방문·바이럴의 거점이 되는" 구조
- `lib/notify.ts`에 `feed_new_post`/`store_follow` 알림 타입 추가, `app/notifications/page.tsx`에 아이콘/색상 매핑 추가
- **DB 패치 미적용 상태** — `supabase/patch_feed_store.sql`을 Supabase SQL Editor에서 실행해야 매장 귀속 글 작성·팔로우 기능이 정상 동작함

**인스타그램 스타일 피드 및 글로벌 플로팅 DM 메신저 연동 완료 (2026-07-13)**
- **인스타그램 스타일 피드 기능 구현** (`app/feed/page.tsx`, `app/api/feed/`):
  - 피드 전체/내주변/매장소식/구직피드 카테고리 필터링 조회 (GET)
  - 피드 등록 (POST), 좋아요 토글 (POST), 댓글 작성 및 삭제 (POST/DELETE), 북마크 저장 (POST) 및 저장 목록 조회 (GET) API 연동
  - 마이페이지 하단 `내 스토리` 및 `저장됨` 3열 그리드 탭 연동 및 모달 라이트박스 팝업 내 삭제/좋아요/댓글 인터랙션 구현
  - 타인 프로필 페이지 하단 `피드 스토리` 그리드 및 모달 팝업 동일 적용
- **글로벌 플로팅 DM 메신저 위젯 통합** (`components/FloatingChatWidget.tsx`):
  - 하단 네비게이션 [채팅] 탭을 [피드] 탭으로 개편함에 따라 우측 하단 플로팅 메시지 버튼(💬)으로 채팅창 진입 동선 설계
  - 버튼 클릭 시 컴팩트한 채팅방 목록 및 개별 채팅 스트림 팝업 토글 (실시간 안읽은 카운트 뱃지 및 Realtime insert/update 반영)
  - 팝업 헤더 영역에 **`(↗️) 전체보기`** 및 **`(↗️) 채팅 상세`** 한글 결합형 익스팬드 버튼 추가 ➡️ 각각 전체화면 대화목록(`/chat`) 및 대화방([`/chat/[id]`](file:///c:/pazabv2/app/chat/%5Bid%5D/page.tsx))으로 원클릭 심리스 진입 제공
- **마이페이지 `전체 채팅 보관함` 대시보드 메뉴 추가** (`app/mypage/page.tsx`):
  - 하단 [MY] 탭에서 `💬 전체 채팅 보관함` 바로가기 카드를 배치하여 2클릭만에 풀스크린 대화창 목록 진입 가능하도록 개선
  - 마이페이지 내 `러브콜/매칭` 카드 내부의 `💬 채팅하기` 버튼 누를 시, full-screen 채팅방(`/chat/[id]`) 다이렉트 랜딩 보장
- **Supabase 데이터 유실(matches, chats 테이블 0건화) 진단 및 복구**:
  - `team_members` 테이블에 등록되어 참조되던 `match_id`들의 부모 `matches` 데이터 복구 스크립트 작성 및 DB 적용 완료
  - 부모 데이터가 삭제되어 RLS 및 쿼리 조건으로 인해 대화 목록이 사라졌던 이슈 진단 및 임시 placeholder 메시지와 함께 대화방 3개 정상 복원 완료

**계약서 OCR 아키텍처 전환 및 UI 정리 (2026-07-11)**
- **OCR 엔진 전환**: PaddleOCR(규칙파서) → EasyOCR → **Claude Haiku Vision**(`claude-haiku-4-5-20251001`) 순으로 전환. 최종적으로 OCR UI를 프론트에서 걷어내되 백엔드 파일은 유지 (향후 재활성화 대비)
  - `ocr-test/app.py`: EasyOCR 기반 FastAPI 데몬 (유지, 연결 해제)
  - `ocr-test/compare_ocr.py`: PaddleOCR vs EasyOCR 비교 스크립트 (유지)
  - `app/api/contract/ocr-haiku/route.ts`: Claude Haiku Vision 직접 호출 라우트 (유지, 연결 해제) — AiGate 50k 문자 제한 우회를 위해 `new Anthropic({ apiKey })` 직접 사용
  - `app/api/contract/ocr/route.ts`: 기존 Python 데몬 프록시 라우트 (유지, 연결 해제)
- **계약서 페이지 레이아웃 단순화** (`app/contract/page.tsx`):
  - BBox 오버레이·OCR 이미지 뷰어·진입 선택 화면 전부 제거
  - 단일 컬럼 레이아웃 (`max-width: 680px`), 폼 위·미리보기 아래 배치
  - 미리보기 기본 펼침(`isPreviewCollapsed: false`), `zoom: 0.75`로 레이아웃 반영 확대
- **이전 계약서 템플릿 불러오기 버그 수정** (`app/contract/page.tsx`):
  - `fetchPrevContract`에 `employerProfileId` 파라미터 추가 → 현재 선택된 매장의 계약서만 조회 (기존: employer 전체 계약서 중 최신)
  - `applyPrevContract`의 `keptFields`에 사업체 필드(`biz`, `bizRegNo`, `ceo`, `ceoPhone`, `bizAddr`, `bizAddrDetail`, `bizType`, `samePlace`, `workPlace`) 추가 → `initF`가 채운 현재 매장 정보를 이전 계약서 데이터로 덮어쓰지 않음. 템플릿에서 복사되는 건 임금·근무시간·요일·보험 등 재사용 조건만

**계약서 OCR 규칙 파서 및 인터랙티브 하이라이팅 연동 (2026-07-10)** *(이후 UI 제거됨)*
- **OCR 규칙 파서 성능 개선**: 역정렬 X/Y 정렬 알고리즘, 이웃 단어 탐색(`find_right_neighbor`), 시급 클렌징 노이즈 필터 적용 → 시급 0% ➡️ 93%, 주소 0% ➡️ 96% 정확도 대폭 향상
- **정밀 Bounding Box 좌표 회복**: 한 글자 조각에 쏠리던 오버레이 역방향 비교 조건(`clean_txt in clean_val`) 제거
- **모바일 최적화**: 220px 컴팩트 뷰어, 양방향 포커스 싱크, 접이식 토글, 라이트박스 팝업 — *이후 레이아웃 단순화로 전부 제거됨*

**버그 수정 및 UX 개선 (2026-07-10)**
- **퇴직자 재초대 데이터 오염 버그 수정**:
  - `InviteBottomSheet` 팀원 존재 체크에 `.eq("status", "active")` 추가 → 퇴직자(`left`) 재초대 가능
  - `app/i/[code]/page.tsx` existing 체크에 `.eq("status", "active")` 추가 → 퇴직 레코드를 재활성화하지 않고 새 `team_members` row INSERT → 이전 계약서·급여명세서 오염 차단
- **이전 근무 이력 매장명 오류 수정**: `loadPastWorks`에 `employer_profile_id` 컬럼 추가 및 해당 ID로 직접 매장 프로필 조회 → 다중 매장 사장님 환경에서 잘못된 매장명 노출 수정
- **이전 근무 급여내역 조회 수정**: `WorkerPayslipTab`에 `teamMemberId` prop 추가, 이전 근무 카드에서 `team_member_id` 기반으로 정확한 급여 내역 인라인 표시 (기존 `/payslip?id=` 잘못된 라우팅 제거)
- **현재 재직 급여명세서 이전 근무분 혼입 방지**: 활성 팀원 `WorkerPayslipTab`에도 `teamMemberId` 전달 → 같은 사장 밑 재취업 시 이전 기간 명세서 섞임 차단
- **payslips FK 오류 수정**: `payslips.match_id` NOT NULL 제약으로 초대 직원 급여 발행 불가 → `patch_payslips_match_id_nullable.sql` 실행 + payload에서 `match_id` 조건부 포함 처리
- **퇴직 처리 로직 개선**: 당월 출근 기록이 0건이면 급여 명세서 없이 바로 퇴직 처리 허용 (기존: 무조건 급여 명세서 필수로 데드락 발생). 퇴직 확인 모달에 이번 달 출근 상태 명시 + 출근 기록 있으면 확정 버튼 비활성화
- **마이팀 매장 선택 상태 유지**: `sessionStorage`에 `activeStoreId` 저장 → 팀원 상세 갔다 돌아와도 선택했던 매장 유지
- **AppHeader 알림·설정 메뉴 확장**: `showBellAndMenu` prop 추가 → 대타·근태(내 근무)·마이팀 헤더에도 알림 벨 🔔 + ⋮ 메뉴 추가 (기존엔 채팅·MY에만 있었음)

**최근 기능 고도화 (2026-07-10)**
- **채용 확정 후 넥스트 스텝 배너**: 채팅방 내부 매칭 상태가 `hired`이고 계약서가 없을 때 사장님/알바생 각각 근로계약서 작성을 안내하고 라우팅 링크를 제공하는 상단 고정 안내 배너 구현
- **초대 직원 러브콜 기록 미발생 차단**: 초대장 기반 matches 데이터를 러브콜 내역 조회 필터링에서 차단하고, 수락 즉시 알바 구직 프로필 `is_active: false, is_public: false, job_status: 'hired'`로 일괄 갱신
- **마이팀 아코디언 디폴트 전개 제어**: 소유 매장 수 및 알바 소속 매장 유무에 따라, 신규 가입자나 양방향 소속자는 둘 다 펼침, 한쪽만 있는 회원은 있는 쪽만 펼치고 빈 쪽은 접어두는 로직 구현
- **직원 퇴사 처리 검증 및 데이터 동결**:
  - 퇴사 처리 시 당월 급여 명세서(payslip)가 생성 완료되었는지 검증해 미발행 시 퇴사 차단하는 안전장치 도입
  - 퇴사한 직원(status = 'left')의 상세 페이지 내 근무조건 수정, 계약서 수정/추가, 수동 급여 발행, PAZ 호칭 저장, 서류 제출 상태 체크박스 비활성화 및 캘린더 날짜 클릭을 통한 과거 근태 수정 전면 차단 (동결)
  - 퇴사일 이후 미래 일자는 출근 예정일에서 제외하고 캘린더 내 퇴사 당일 칸에 `🚪 퇴직` 시각 표시
- **알바생 경력 관리 및 서류 영구 열람 보장**:
  - 알바생 마이팀 "내 소속" 하단에 "🏛️ 이전 근무 이력 (경력)" 섹션을 구현하여 퇴사한 매장명, 근무 기간, 업종, 직책 아카이브
  - 카드 내 `📄 근로계약서 조회` 및 `📋 급여내역 조회` 읽기 전용 버튼을 연동하여 과거 발행된 서류에 대한 영구 조회 보장
  - 알바생 "내 근무"(`worker/mywork/page.tsx`)의 히스토리 탭에도 동일한 서류 열람 바로가기 버튼 추가
- **팀원 초대 매장 자동 pre-select 연동**: `InviteBottomSheet`에 `defaultStoreId` props를 추가하고 마이팀에서 활성화된 activeStoreId를 전달하여 팝업 실행 시 해당 매장이 자동으로 디폴트 선택되도록 개선

**인증**
- createBrowserClient(@supabase/ssr) 전환, 서버사이드 OAuth Route Handler(/api/auth/login, /api/auth/callback) 적용 → 모바일 PKCE 루프 해결
- proxy.ts(구 middleware.ts) PUBLIC_PATHS 정리

**온보딩**
- 3필드/역할지연 설계 폐기 → 2버튼(사장님/알바생) 즉시 진입으로 확정

**사장님 다중업장**
- employer_profiles UNIQUE 제약 제거 + biz_reg_number/ceo_name/biz_address/biz_tel 컬럼 추가 (patch_employer_multi_biz.sql 실행 완료)
- 계약서 업장 자동선택 + 정보 자동채움 (app/contract/page.tsx)
- 주소정책 확정: address(도로명 전체, 계약서용) / region(3단계, 공개UI용) / lat,lng
- users 테이블 닉네임 unique index (대소문자 무시) 적용 완료 (patch_nickname_unique.sql)

**초대(공급 확보)**
- 닉네임 검색 기반 직접초대로 전환(app/invite/page.tsx), 매니저 역할(team_members.member_role) 추가
- 초대수락(app/i/[code]) → team_members 연결 + worker_profiles 자동승격

**근태**
- GPS 200m 반경 출근 제한(Haversine), 지각/정시 판정
- attendance RLS 재작성 완료 (worker_id/employer_id 직접 비교)
- 5분 주기 크론(app/api/cron/checkin) — 출근10분전/정시/5분지각/20분결근자동처리 알림, cron-job.org 외부연동

**UI**
- myteam/mypage 탭 제거 → 단일스크롤+아코디언, 색상체계 통일(알바=핑크dominant/사장=보라dominant)
- 네비 5탭 확정: 탐색·대타SOS·[근태(중앙)]·채팅·MY
- TimeWheelPicker/DateWheelPicker 신규(네이티브 input 대체)
- 알림시스템(lib/notify.ts, NotificationBell, /notifications)

**UI 디자인 전면 개선 (2026-07-09)**
- 채팅 목록(`/chat`): 구분선 방식 → 카드 방식(border-radius 18), 읽지 않은 방 강조(보라 링+굵은 텍스트), 아바타 온라인 도트, 카드 hover lift 효과
- 채팅방(`/chat/[id]`): 메시지 버블 shadow 추가, 날짜 구분선 `—— 날짜 ——` 방식, 입력창 포커스 glow, 전송 버튼 그라디언트/shadow, 헤더 full-width 구조 개선
- 채팅방 헤더: ⋮ 메뉴에 사전미팅 이동, 계약서 버튼 아이콘 전용 원형 버튼, 뒤로가기 원형 버튼, 아바타 온라인 도트
- AppHeader: 대타SVG·스도쿠SVG 인라인 제거 → ⋮ 드롭다운(대타SOS/스도쿠/설정)으로 이동, 뒤로가기 `ti-chevron-left` 원형 버튼 통일, 헤더 아바타 제거(네비 MY탭으로 충분), blur 강화
- BottomNav: 액티브 pill 배경 제거 → 상단 그라디언트 바 인디케이터, 중앙 근태 버튼 액티브 ring glow, 뱃지 그라디언트, safe-area-inset-bottom 적용
- 설정 페이지: "내 정보" 섹션(이름/전화/생년월일/지역) 제거 — 실명·연락처·생년월일은 계약서 작성 시점에만 수집, settings는 계정·알림·정보·위험구역만 유지

**UI/색상 시인성 전면 개선 (2026-07-02 3차)**
- globals.css에 라이트/다크 통합 CSS 변수 추가: `--success-text`, `--purple-text`, `--pink-text`, `--card-inner`, `--card-inner-border`, `--progress-track`, `--chip-*-bg/border` 등 (라이트=진한색, 다크=파스텔 유지)
- mypage 전체: `#86efac`(연초록)·`#c4b5fd`(연보라)·`#f9a8d4`(연핑크) → var() 교체로 흰배경 시인성 확보
- 종합 신뢰도 게이지 배경 `rgba(255,255,255,0.02)` → `var(--surface2)`, progress track → `var(--progress-track)`
- 사장님 공고 카드 배경 `rgba(0,0,0,0.2)` → `var(--surface2)`, border → `var(--border)`
- 봇설정·수정·공고등록 등 버튼 색상 전부 var() 통일
- "내 프로필 카드 미리보기" 버튼 제거 (미완성 기능, state도 정리)

**빈 매장/공고 노출 버그 수정 (2026-07-02 3차)**
- 온보딩에서 사장님 선택 시 빈 employer_profiles row 자동 INSERT하던 로직 제거 (app/onboarding/page.tsx)
- myteam loadTeam 쿼리에 `.not("business_name", "is", null)` 필터 추가 → 불완전 매장 카드 노출 방지
- mypage fetchJobs 및 미리보기용 eps 쿼리에 동일 필터 추가 → "설정안된거 1개" 노출 방지

**탐색 페이지 당근 스타일 FAB (2026-07-02 3차)**
- 공고 탭: `[+ 새 공고]` (보라) → `/employer/register`
- 구직자 탭: `[+ 새 구직]` (핑크) → `/worker/profile?edit=true&new=true`
- scrollY > 80 → 원형(+) 으로 수축, scrollY ≤ 80 → 필 모양 텍스트 노출 (max-width 트랜지션)
- 위치: `bottom: 92px` (BottomNav 위, PAZ 기본위치 아래)

**내팀 다중 매장 UI (2026-07-02 2차)**
- 로그인 후 첫 페이지 라우팅: employer→/myteam, worker→/explore
- StoreRegisterModal (components/StoreRegisterModal.tsx): 매장 등록/수정 팝업, Kakao 상호명 검색(업종/주소 자동입력), Daum 우편번호, 미디어 업로드(사진10/영상1), 직접입력 토글
- Supabase Storage 버킷명 avatars→media 전체 교체 (employer/register, mypage, worker/profile)
- Samsung Pass 스타일 롤로덱스 카드 UI (app/myteam/page.tsx): 비활성 매장은 헤더 한 줄(52px)만 절대위치로 쌓이고, 탭 누르면 맨 아래로 내려와 전체 펼침
- 매장 삭제 버튼(✕) + 확인 팝업 구현
- patch_missing_columns.sql: employer_profiles/users/worker_profiles/team_members/attendance 누락 컬럼 전체 추가, attendance_logs/notifications/tax_rates/push_subscriptions 테이블 신규 생성

**근로계약서 UX 개선 (2026-07-02)**
- 탭 방식 → 5단계 스텝 위자드로 전환 (사업체→근로자→근무→임금→보험·서명)
- 진행바 + 완료 스텝 ✓ 표시, 스텝 아이콘 직접 클릭 이동 가능
- 모바일 최적화: 날짜(type=date), 시간(type=time) 네이티브 피커 적용
- 담당업무 preset 버튼 6종(홀서빙/주방/카운터/청소/배달/재고) + 직접입력
- 지급일 버튼 선택(5/10/15/20/25/말일), 임금 구분/지급방법 큰 버튼화
- 보험 4종(고용/산재/연금/건강) 아이콘 카드 터치 방식
- 근로자 주소 검색 후 세부주소(동·호수·층) 입력란 자동 노출
- workerAddrDetail 필드 추가, 저장 시 전체주소 합산 저장

**급여/세금 (설계만 완료, 코드 미착수)**
- 일용근로소득세 공식 확정: (일급-15만)×6%×45%, 지방세 10%, 소액부징수 <1천원
- 4대보험 판정기준 확정, tax_rates 테이블(연도별 INSERT-only) 설계 확정

## 발생했던 이슈 (해결됨, 재발 방지용 기록)

| 이슈 | 원인 | 해결 |
|---|---|---|
| 새 계정(hellopazab) 내팀에 "업종미정" 빈 카드 노출 | onboarding에서 사장님 선택 시 business_name 없는 빈 employer_profiles row INSERT | onboarding 자동 INSERT 제거 + 쿼리에 `.not("business_name","is",null)` 필터 |
| 마이페이지 "설정안된거 1개" 공고 노출 | 위와 동일한 빈 row가 fetchJobs에도 노출됨 | fetchJobs 쿼리 동일 필터 추가 |
| 라이트모드 텍스트 시인성 불량 | `#86efac`·`#c4b5fd`·`#f9a8d4` 등 다크모드 전용 파스텔 색을 하드코딩 | globals.css에 `--success-text` 등 라이트/다크 분기 CSS 변수 추가 후 전체 교체 |

| 이슈 | 원인 | 해결 |
|---|---|---|
| 모바일 로그인 무한루프 | localStorage 기반 세션+PKCE verifier, 인앱브라우저 전환시 유실 | 서버사이드 OAuth 이전, 쿠키 저장 |
| attendance INSERT 실패 | RLS가 team_members.user_id 참조(없는 컬럼) | worker_id/employer_id 직접비교로 재작성 |
| myteam 팀원 0명 표시 | select에 없는 컬럼(match_id) 포함 → PostgREST 전체 null 반환 | select에서 제거 |
| /invite 사장님 UI 접근불가 | user_type=='employer' 하드게이트 | 의도기반 탭 전환(viewMode)으로 변경 |
| employer_profiles 미생성 | 온보딩이 onboarding_data JSONB에만 저장, 테이블 insert 누락 | 온보딩 완료시 자동 INSERT 추가 |
| 로그인 재진입시 signOut 유발 루프 | login 페이지 마운트시 signOut() 호출 | 해당 호출 제거 |
| Claude Code 워크트리 오작동 | 세션이 worktree 컨텍스트에서 시작 → 메인 프로젝트가 아닌 워크트리에 파일 편집 | .claude/settings.json에 bgIsolation:none 설정 |
| 매장 삭제 RLS 차단 | employer_profiles DELETE 정책 미생성 | `CREATE POLICY employer_profiles_delete FOR DELETE USING (auth.uid() = user_id)` 실행 |
| Supabase Storage 업로드 실패 | 코드가 "avatars" 버킷 참조, v2 프로젝트엔 "media" 버킷만 존재 | 전체 코드에서 "avatars"→"media" 교체, media 버킷 RLS 정책 추가 |
| employer_profiles 컬럼 없음 오류 | v2_schema.sql이 최소 구성이라 코드에서 참조하는 컬럼 대부분 누락 | patch_missing_columns.sql 작성 및 실행 |

**구현 완료 (2026-07-03)**

**라이트모드·UX 버그 수정**
- globals.css `--nav-bg` / `--nav-border` / `--search-bg` / `--search-border` CSS 변수 추가 (라이트=흰배경, 다크=기존 유지)
- 탐색 헤더·검색창 배경 하드코딩 → var() 교체
- 탐색 FAB user_type 체크: 알바생이 공고탭 FAB 누르면 구직 프로필로, 사장님이 구직탭 FAB 누르면 공고 등록으로

**employer_profiles is_deleted null-safe 전체 수정**
- myteam / mypage / invite / contract 페이지 `.eq("is_deleted", false)` → `.or("is_deleted.is.null,is_deleted.eq.false")` 전환
- 원인: patch_missing_columns.sql로 추가된 컬럼 기존 row의 기본값이 null이라 필터에서 걸러짐

**매장 삭제 안전장치 + 팀원 퇴사처리**
- 매장 삭제 하드 DELETE → soft delete (`is_deleted:true, is_active:false`)
- 팀원 있으면 삭제 모달에 명단 표시 + "퇴사처리 후 삭제" 원클릭
- 팀원 카드에 개별 "퇴사" 버튼 추가 (status='left')

**직원 서류 보관함 신규 (app/employer/records)**
- 전·현직 팀원 전체 목록 (재직/퇴사 필터)
- 개인별: 근무조건·계약서 목록(서명상태)·월별 근태기록
- 법정 보존기간 표시 (3년/5년) + 만료일 색상 경고
- 내팀 하단 "📂 직원 서류 보관함" 진입 버튼

**계약서 위자드 개선**
- 담당업무 다중선택 (쉼표 join 저장), 프리셋 2종 추가 (음료 제조·포장마감)
- 근무요일 선택 시 주휴일 자동 추천 (첫 번째 쉬는 날)
- 주휴일 라벨 → "주휴일 (유급 휴무일 · 하루치 추가 지급)"
- 주휴일 버튼: 근무일 흐리게·쉬는날 보라색 강조
- 보험 카드 미선택 시 `color:"#fff"` → `var(--text)` (라이트모드 대응)
- 연장근로·야간근로·주휴수당 자동계산 패널 (임금 스텝)
  - 주 연장 12h 초과 시 빨간 경고 (근로기준법 제53조)
  - 최저임금 미달 시 경고 + 벌칙 안내
- 스텝4 첨부서류 확인 체크리스트: 보건증·신분증·통장사본·친권자동의서

**최저임금 동적 관리 (lib/minWage.ts)**
- 연도별 최저시급 테이블 (2024~2026)
- 근무 시작일 기준 적용, 계약서 전체 하드코딩 제거
- tax_rates DB 구현 후 마이그레이션 예정

## 다음 작업 (우선순위순)

**P0**
- [x] tax_rates 2026 초기값 입력 (건강보험 3.545%, 고용보험 0.9%, 국민연금 4.5%)
- [x] job_credentials 테이블 생성 + 시드 데이터 INSERT
- [x] min_wages 테이블 생성 + 2024~2026 초기값
- [x] 탐색 FAB `userType === "both"` 케이스 — 코드 확인 결과 이미 탭 기준 올바르게 처리됨

**P1**
- [x] lib/taxRates.ts — calcDailyWorkerTax(), calcInsuranceEligibility(), calcInsuranceDeduction()
- [x] lib/minWage.ts — DB 연동(fetchMinWage) + 동기 폴백 유지
- [x] app/admin/tax-rates/page.tsx — hellopazab 전용, 세율+최저임금 CRUD
- [x] 계약서 진입점 재설계 — memberId 파라미터, contracts 전체 team_member_id 기반 전환
- [x] team_members.employer_profile_id 연결 — 초대 수락(/i/[code]) 시 invite_codes.employer_profile_id 반영
- [x] 근태 전체보기 — /employer/team/[id] 월별 필터 + DB 재조회 (viewYear/viewMonth 기반)
- [x] 매장 등록 ↔ 공고 등록 분리: StoreRegisterModal is_active:false, 내팀 "📢 공고올리기" 버튼, employer/register storeId 파라미터
- [x] 매장 삭제 안전장치 — openDeleteModal DB 직접 조회(employer_id 기반), 팀원 있으면 삭제 버튼 숨김
- [x] mypage 공고 마감 — is_deleted 제거, is_active:false만
- [x] 사장님 팀원상세 /employer/team/[id] 근무조건 수정 기능
- [x] employer_profiles(매장) ↔ jobs(공고) 완전 분리 아키텍처 — jobs 테이블 신규, match/lovecall/mypage/myteam/contract/job/[id]/paz-register 전체 반영, DB: db/patch_jobs_table.sql
- [x] job_categories 테이블 생성 — 10개 대분류·46개 소분류 (음식점/카페/편의점/패스트푸드/마트/물류창고/생산제조/건설노무/의료복지/기타), 알바몬·알바천국 대비 3개 추가
- [x] job_credentials 직업별 필수서류 DB 내재화 — category_name+duty_name+name+is_mandatory_by_law 구조
- [x] 팀원 상세(/employer/team/[id]) 기능 다수 추가:
  - 근무 예정일 달력 시각화 (work_days 파싱 → 배경색+예정 뱃지)
  - "N일 중 M일 출근" 통계 표시
  - 계약서 없을 때 필요성 안내 카드 + 급여명세 발행 잠금
  - 📂 서류 현황 아코디언 — job_credentials 기반 동적 서류목록, 제출여부 DB 즉시저장 (team_members.docs_submitted JSONB)
  - 보건증 필요여부 business_type → job_credentials DB 조회, 미성년자 친권자동의서 자동 노출
- [x] 계약서 위자드 추가 개선:
  - 출퇴근 시간 → 1일/1주 소정시간 자동계산 + 휴게없음 체크박스
  - 계좌이체 선택 시 계좌번호 입력 (선택)
  - 담당업무 preset → job_categories DB 소분류 동적 로드
  - 서류 섹션 "미제출이어도 계약서 작성 가능" 안내 추가
- [ ] 직원 서류 보관함에 payslip 섹션 추가 (급여 구현 후)
- [x] team_members.docs_submitted 컬럼 추가 SQL 실행 완료
- [x] job_categories RLS SELECT 정책 추가 (미설정으로 구직 희망직종 안 뜨던 버그 수정)
- [x] 마감된 공고 재표시 버그 수정 — 마감하기 → job_status:'closed', fetchJobs에서 closed 제외
- [x] users 허브화 (공통 데이터 단일 입력):
  - DB: users 테이블에 region/address/birth_date 컬럼 추가
  - settings 페이지: 이름/연락처/생년월일/거주지역 한 곳에서 수정
  - worker/profile 신규 등록 시 users.region → 희망지역 자동채움
  - employer/register 신규 매장 등록 시 users.region → 지역 자동채움
  - lib/regions.ts 신규: REGIONS 상수 공용화 (employer/register, worker/profile 중복 제거)

**구현 완료 (2026-07-06)**

**근로계약서 작성 위자드 입력 로직 개선**
- 계약서 작성 첫 진입 시점부터 **출근 09:00 / 퇴근 18:00** 기본값이 셋업되도록 초기 폼 상태(`f`)와 DB 로딩 함수(`initF`)를 수정했습니다.
- 이에 따라 1일/1주 소정근로시간이 진입하자마자 **8시간 / 40시간**으로 자동 계산되어 사용자가 시/분 조정만으로 빠르게 계산이 이뤄질 수 있도록 개선했습니다.
- 사업체 정보 대표 연락처(`ceoPhone`)에 걸려 있던 휴대폰 번호 전용 정규식 유효성 검사 규칙을 제거하여 일반 유선전화나 070 번호도 자유롭게 기입할 수 있도록 예외처리했습니다. (단, 알바생 본인 연락처 `workerPhone` 은 알림 연동을 위해 휴대폰 포맷 유지)

**미성년자 생년월일 경고 문구 강화**
- 근로자 생년월일 입력 시 만 18세 미만일 때의 경고 안내 메시지에 "보호자(친권자/후견인) 동의서 작성이 법적 필수"임을 명시하고 마지막 단계에서 보호자 입력란이 활성화됨을 구체적으로 알려주어 사장님이 당황하지 않고 마법사를 이어갈 수 있도록 보강했습니다.

**계약서 수정 요청 모달 커스텀화**
- 계약서 동의 처리 창에서 직원이 '⚠️ 수정 요청' 버튼을 클릭할 때 뜨던 브라우저 기본 내장 다이얼로그(`prompt`)를 흐릿한 블러 효과(`backdropFilter: "blur(6px)"`)가 가미된 세련된 UI의 **커스텀 리액트 입력 모달창**으로 전면 교체했습니다.

**채팅방 계약서 수정 버튼 연동 버그 수정**
- 채팅창 내에 있는 '수정하기', '계약서 수정하기', '재계약하기' 등 사장님이 직원의 계약서를 보정하는 모든 라우팅 경로를 헬퍼 함수(`goToUpdateContract`)로 일체화했습니다.
- 이를 통해 단순히 `matchId`만 보내던 기존 주소창 파라미터에서 `team_members` 테이블을 참조하여 **`memberId` 파라미터를 정확하게 매핑**해 줌으로써, 직원 상세 페이지의 수정 버튼과 동일하게 원본 계약서를 완벽히 보존하며 부분 수정으로 이어지도록 연동을 완료했습니다.

**구현 완료 (2026-07-05)**

**근로계약서 플로우 전면 정비**
- 계약 타입(표준/단시간/연소자)에 "요일마다 달라요" 토글 추가 (parttime 제외)
- `contracts` 테이블 누락 컬럼 추가: `contract_data` (JSONB), `work_days` (text)
- `buildPayload()` 수정: `employer_signed_at`, `contract_type`, `duties`, `workplace_address` 포함
- 계약서 저장 전 확인 모달(바텀시트): 근로자·계약유형·임금·시작일·근무요일 미리보기
- 계약서 서명(`window.confirm`) → 바텀시트 확인 모달로 교체
- 계약서 저장 시 `team_members.contract_status = "pending"` + 매칭 `status = "accepted"` 자동 승격

**계약서 발행 취소 및 상태 동기화**
- `/employer/team/[id]`, `/myteam`, `/chat/[id]` 세 화면 모두 발행 취소 지원
- 취소 시 `contracts.status = "cancelled"` + `team_members.contract_status = "none"` 동시 업데이트
- "서명대기" 뱃지 취소 후 즉시 소멸 (로컬 state 직결)

**계약서 서명 server-side 처리 (`/api/contract` PATCH)**
- 알바생 계약서 서명 시 `team_members.contract_status` 업데이트가 anon RLS에 막히던 문제 해결
- `/api/contract` PATCH 핸들러 신규: `action:"sign"` — contracts/team_members/matches 모두 service role로 일괄 업데이트
- team_member_id 없을 시 match_id+worker_id로 폴백 조회

**채팅·매칭 버그 수정**
- `/api/chat` POST: anon key → service role key로 교체 (chats INSERT RLS 차단 해결)
- `/api/chatrooms` 전면 재작성: 비존재 컬럼(`last_message`, `chat_opened`, `employer_profile_id`) 참조 제거, chats 테이블 직접 집계
- Realtime 채널명 `Date.now()` 접미사로 중복 채널 오류 해결
- REPLICA IDENTITY FULL: matches·chats 테이블 적용 완료 (Supabase SQL 실행)
- `/api/lovecall`: anon key → service role key (`worker_left` 업데이트 RLS 해결)
- `proxy.ts` PUBLIC_PATHS: 개별 `/api/auth`, `/api/cron` → `/api` 전체로 단순화 (middleware 누락 차단 방지)

**채팅방 나가기 안정화**
- 클라이언트 `supabase.from("chats").insert` → `/api/chat` POST 경유로 교체 (RLS 우회)
- `/api/lovecall` `case "leave"`: 중괄호 블록으로 const 스코프 명확화, DB 오류 콘솔 로깅 추가
- 나가기 응답 체크: fetch 실패 시 `console.error`로 기록

**`team_members` 단일 진실 원천 확립**
- contract_status 컬럼이 실질적 계약 상태 단일 source of truth로 확립
- 모든 저장/서명/취소 경로에서 team_members.contract_status 동시 업데이트 보장

**구현 완료 (2026-07-08)**

**계약서 발행 버튼 무반응 수정**
- `/employer/team/[id]` "새 계약서 작성" 버튼이 `matchId` 기반 라우팅이라 `load()`에서 `progress_status:"hired"` 필터에 걸려 `selMatch`가 null → 발행 눌러도 아무 반응 없는 버그 수정
- `memberId` 파라미터 기반 라우팅 (`/contract?memberId=${member.id}`)으로 통일
- `saveContract()`에 `!selMatch` 시 토스트 + `try-catch` 에러 처리 추가

**계약서 근로자 정보 미로딩 수정**
- `loadByMember()` / `load()` / `handleSelectMatch()` 세 경로 모두 users 쿼리에 `address_detail` 누락 → 추가
- `initF()`에서 `workerAddr: ""` / `workerAddrDetail: ""` 하드코딩 → `wu?.address || ""` / `wu?.address_detail || ""`로 수정
- 계약서 저장(`doSave`) 시 근로자 정보(이름/생년월일/연락처/주소)를 `users` 테이블에 역sync → 이후 계약서 작성 시 자동 채움

**계약서 임금 타입 저장 수정**
- `buildPayload()`에서 `wage_type` 컬럼 누락 → `wageType`(hour/day/month) → DB 포맷(hourly/daily/monthly) 변환 추가
- `validateStep` case 3: `const` in switch 스코프 오류 → 블록 `{}` 래핑, 시급/일급/월급 각각 시급 환산 후 최저임금 비교

**알림·채팅 배지 실시간 반영 수정**
- `NotificationBell`: Realtime `postgres_changes` 구독에 `filter: user_id=eq.${uid}` 누락 → 추가
- `BottomNav`: 채팅 배지가 폴링만 하고 Realtime 없던 구조 → `chats` 테이블 INSERT/UPDATE Realtime 구독 추가
- `notifications` 테이블 `REPLICA IDENTITY FULL` 적용 완료 (patch_notifications_realtime.sql)

**월별 소급 등록 버튼 위치 이동**
- 계약서 섹션에 있던 "📅 월별 소급" 버튼을 근태 관리 월 선택 행 우측으로 이동 ("📅 이달 소급")
- 서명 완료된 계약이 없으면 버튼 미노출, 있을 때만 표시

**팀원 상세 예상 급여 계산 타입별 분기**
- 기존: 항상 `총시간 × 시급` 계산 → 월급 설정인 경우 8,640,000원 같은 엉뚱한 값 표시
- `loadMember()`에서 `contract_data.wageType` 읽어 `member.wage_type`으로 저장
- **시급(hourly)**: 기존 `총시간 × 시급 + 초과 × 1.5` 유지
- **일급(daily)**: `출근일수 × 일급 + 초과시간 × (일급/contractHours) × 1.5`
- **월급(monthly)**: `월급 × (출근일/예정출근일)` 일할 계산 + 초과분은 209시간 기준 시급환산 × 1.5
- 급여 카드 상단 설명도 wageType에 맞게 동적 표시 (출근일/일수/시간)

**근태 관리 및 감사 로그 고도화 (2026-07-05 2차)**
- **근태 삭제 RLS 정책 누락 보완**: 사장님이 소속 팀원의 근태 데이터를 안전하게 제거할 수 있도록 `attendance` 테이블의 DELETE RLS 정책 추가 (`supabase/patch_delete_policy.sql` 실행 완료).
- **커스텀 삭제 확인 모달 도입**: 브라우저 기본 `confirm()` 경고창을 유리모핑 스타일의 프리미엄 경고 모달 다이얼로그로 대체.
- **최근 3건 타임라인 & 페이징 모달 이력 뷰어**: 달력 하단 최근 3건 근태 로그 요약 노출 및 3건 초과 시 10개 단위 페이징(Pagination)이 적용된 전체 이력 조회 모달창 탑재.
- **⚡ 근태 일괄 등록 (소급 발행 지원)**: 특정 월의 계약 약정 요일들 중 오늘 날짜 이전의 근무일들을 일괄 '정상 출근'으로 원클릭 기입하는 기능 구현.
  - 기존 근태 기록 덮어쓰기 토글 지원 (체크 해제 시 기존 수동 입력분 보호, 체크 시 일괄 덮어쓰기).
  - 모달 설명 레이아웃 개편: 텍스트 볼드 및 경고 박스 가독성 전면 개선.
- **감사 로그 컬럼 누락 해결 및 요약 로깅**: `attendance_logs` 테이블의 `attendance_id` 누락 컬럼 보완 패치 생성 (`supabase/patch_attendance_logs_column.sql`) 및 일괄 등록 시 timeline을 해치지 않고 `batch_register` 액션 단일 감사로그 1건으로 요약 등록되도록 구현.
- **커스텀 토스트 알림 표준화**: 모달 내의 브라우저 기본 `alert` 경고들을 모두 파잡 전용 커스텀 토스트 알림(`showToast`)으로 통일.

**구현 완료 (2026-07-09)**

**임금 타입별 예상 급여 & 세후 실수령액 표시**
- `app/myteam/page.tsx`: `loadMyWork()`에서 최신 활성 계약의 `contract_data.wageType` 읽어 `wage_type` 저장. useEffect에서 taxRates 로드 후 시급/일급/월급 각 계산 방식으로 `estPay`(세전) + `netPay`(세후) 산출
- 알바생 카드에 "💰 예상 실수령액 (세후)" 표시 추가. 임금 유형 접두어 동적 표시 ("시급"/"일급"/"월급"/"주급")
- `app/employer/records/page.tsx`: `Member` 타입에 `wage_type` 추가, member 초기화 시 contract 기반 동적 속성 반영
- `app/employer/team/page.tsx`: 팀원 목록 로드 시 contracts 조회 후 wage_type 동적 반영, 임금 접두어 카드에 표시

**계약서 미리보기 핀치줌 + 자동 맞춤**
- 미리보기 모달 열릴 때 `window.innerWidth` 기준으로 A4(794px) 비율 자동 scale 계산 → 모바일에서 가로 스크롤 없이 전체 보임
- 두 손가락 핀치 제스처로 0.3x~2.5x 자유 확대/축소 (`onTouchStart/Move/End` + `transform: scale`)

**계약서 인쇄/PDF UX 전면 개선**
- 서버 PDF 생성(Python overlay 방식) 폐기 — `docs/standard_contract_form.pdf` 파일 의존 구조라 실패했던 방식
- `app/contract/view/page.tsx`: "📄 화면인쇄" / "📥 PDF 저장" 버튼 클릭 시 전체화면 미리보기 모달 오픈 → 상단 "🖨️ 인쇄 / PDF 저장" 버튼으로 `window.print()` 호출
- `components/ContractOfficialForm.tsx`: `@page { margin: 18mm 15mm 15mm }` + `.of { width: 100%; padding: 0 }` 인쇄 CSS → 브라우저 여백이 실질 여백 담당, `min-height: 297mm` 제거로 하단 공백 제거
- 제목 `letter-spacing: 4px → 2px`, `font-size: 14pt → 13pt` 조정

**P2**
- [x] HEXACO 5턴 압축 CTA화 — app/interview/page.tsx 신규 구현 (5턴 채팅 UI, /api/analyze interview+analyze 모드 연동, 결과 localStorage+DB 저장, /result로 이동)
- [ ] employer_profiles.geo_radius_meters (GPS 반경 200m 고정 → 사장님 설정 가능하게)
- [ ] 웹푸시 실사용 테스트 미완료
- [ ] 대타 SOS nearby_workers RPC 실연동 점검
- [ ] worker_type(일용/단시간상용) 자동판정 vs 수동선택 미결정
- [ ] 딥링크(/d/[code]) 카톡공유 SOS 미착수
- [ ] worker_profile에 외부 경력 입력 기능 (이전 직장)

**보류/전략만 확정**
- 대타 2-Tier(검증✅/신규🔵) 풀 로직: worker_profiles.is_verified 캐시 설계는 됐으나 코드 미구현
- 급여 자동이체: 토스페이먼츠 API 연동 필요 (사업자 계약 선행)
- 주민번호/통장사본 DB 저장: 암호화 설계 후 P3으로

---

**구현 완료 (2026-07-18)**

**사장님→알바생 채용 제안 흐름 전면 정비**
- **보낸 채용제안 마이페이지 미표시 버그 수정** (`app/api/lovecall/route.ts`):
  - GET 필터 `job_id || daeta_posting_id`만 체크해 사장님 직접 제안(job_id=null)이 목록에서 누락되던 버그 수정
  - `|| employer_interest || worker_interest` 조건 추가 → interest 둘 다 false인 초대 매칭만 제외
- **채용 제안 시 매장 선택 UI** (`app/worker/[id]/page.tsx`, `app/explore/page.tsx`):
  - 사장님이 구직자 상세 또는 탐색 모달에서 제안 시 확인 모달에 매장 선택 버튼 리스트 표시
  - `.limit(1)` 제거 → 사장님의 비삭제 매장 전체 로드, 2개 이상이면 선택 UI, 1개면 매장명 텍스트
  - 선택한 `employer_profile_id`가 API로 정상 전송
- **알림 URL 수정** (`app/api/lovecall/route.ts`):
  - 사장님 채용 제안 알림 URL: `/store/{employerProfileId}` (job_id 없는 경우) → 알바생이 클릭 시 매장 홈으로 이동
- **매장 홈 수락/거절 하단 바** (`app/store/[id]/page.tsx`):
  - 진입 시 `employer_interest=true & progress_status=pending` 매치 조회
  - 해당 매치 있으면 하단 고정 보라 바 + "거절하기 / 수락하기 →" 버튼 표시
  - 수락 → 시스템 채팅 메시지 발송 후 채팅방 이동, 거절 → 바 소멸
- **마이페이지 수락/거절 버튼 버그 수정** (`app/mypage/page.tsx`):
  - `LoveCallSection`에서 `lc.status === "pending"` → `(lc.progress_status || lc.status) === "pending"` 수정
  - API 응답 필드명(`progress_status`)과 불일치로 버튼이 아예 안 보이던 근본 버그 해결
  - pending 뱃지 카운트도 동일하게 수정
- **수락 시 사장님 알림 + 마이페이지 실시간 팝업** (`app/api/lovecall/route.ts`, `app/mypage/page.tsx`):
  - PATCH accept case에 사장님 알림 발송 추가 (`🎉 채용 제안 수락!` → 채팅방 URL)
  - 마이페이지에 `matches` 테이블 Realtime 구독 추가: `employer_interest=true & progress_status=accepted` 감지 시 matchModal 팝업 자동 표시

---

**구현 완료 (2026-07-14)**

**공고 등록 급여 형태(시급/일급/월급) 고도화 및 계산기 연동**
- **다양한 급여 방식 지원**: 공고 등록 화면(Step 2)에 시급, 일급, 월급을 선택할 수 있는 탭을 추가하고, 선택에 맞춰 입력 단위를 동적으로 대응(`원/시간`, `원/일`, `원/월`).
- **예상 급여 실시간 계산기 세분화**: 각 급여 형태(시급/일급/월급)에 최적화된 주 예상 급여 및 월평균 급여(세전) 실시간 연산 구현. (시급일 경우에만 최저임금 필터 및 주휴/야간수당 가산 연산 수행).
- **데이터베이스 호환성**: 추가적인 스키마 마이그레이션 없이 기존 `tags` 배열에 `wage_type:hourly`, `wage_type:daily`, `wage_type:monthly` 형태의 메타데이터 태그를 삽입하는 방식으로 하위 호환성 확보.
- **상세 페이지 동적 라벨**: 공고 상세 화면에서 급여 태그 정보를 읽어 `시급`, `일급`, `월급`을 가변적으로 표시하도록 보완.

**급여 입력값 리셋 및 UX 개편**
- **초기 기본값 제거**: 급여 입력란의 기본 세팅값인 최저임금을 비우고 빈 문자열(`""`)로 설정하여 초기 단계에서 불필요한 예비 계산을 수행하지 않도록 수정.
- **형태 전환 시 자동 초기화**: 사장님이 급여 형태 탭을 전환하면 기존의 입력 금액을 즉시 리셋 처리하여, 시급 단가가 일급/월급으로 이전되어 발생하는 인지적 착오 및 계산 버그를 원천 차단.

**복지 칩 리스트 단일화**
- **독립형 중복 버튼 축소**: 화면의 공간을 넓게 차지하던 `식사 제공` 및 `주차 가능` 독립 버튼을 제거하고, 아래의 칩 태그 리스트(`PRESET_TAGS`)로 통합 단일화.
- **컬럼 바인딩 정합성 보장**: 저장 시에는 선택된 복지 태그 유무를 검출해 `meal_provided` 및 `parking` 컬럼을 갱신하고, 불러올 시에는 컬럼값을 토대로 해당 태그 칩이 켜지도록 양방향 동기화 구현.

**공고 등록 UNIQUE 제약 조건 충돌 및 로딩 오류 해결**
- **이름 없는 대기 매장 로딩 허용**: 백엔드 가입 트리거 등으로 자동 생성된 빈 매장 프로필(`business_name: null`)이 필터로 인해 누락되던 버그를 고치기 위해 `.not("business_name", "is", null)` 필터를 제거하고 `등록 대기 매장` 등의 명칭으로 정상 로드되도록 변경.
- **INSERT 대신 UPDATE 처리**: 저장 시 기존 매장 프로필이 DB에 이미 등록되어 있다면 신규 삽입 대신 기존 행을 갱신(`UPDATE`)하도록 안전장치를 적용하여, `user_id` 고유 키 제약 조건 위반 오류 완벽 해결.
- **매장 프로필 활성화**: 공고를 성공적으로 발행하면 연동된 매장 정보(`employer_profiles`)의 `is_active` 플래그를 자동으로 `true`로 켜서 노출성을 보장.

**공고 확인 커스텀 모달 및 탐색 리다이렉트**
- **검토용 요약 모달 신설**: 최종 등록/수정 완료 전, 매장명, 업종, 위치, 급여 형태/금액, 근무요일 및 시간 등의 정보를 한눈에 보여주는 블러 처리된 커스텀 확인 팝업 모달 추가.
- **성공 피드백 단순화**: 기존의 번거로운 등록 완료 성공 페이지를 건너뛰고, 모달 검토 완료 시 즉시 메인 '탐색(Explore)' 피드로 리다이렉트되어 사용자 플로우 단축.

**탐색 피드 컬럼 오류 및 RLS/이름 버그 해결**
- **jobs 테이블 조인 교정**: 비로그인/예비 탐색 데이터를 불러오는 `loadRaw` 쿼리가 `employer_profiles` 테이블을 향해 없는 컬럼(`job_type`, `expires_at`)을 물어봐 에러로 조회가 멈추던 버그를 `jobs` 테이블 조인으로 전면 정비.
- **name → real_name 일괄 치환**: `users` 테이블에 부재하는 `name` 컬럼 대신 올바른 실명 필드인 `real_name`을 쿼리하고 매핑하도록 탐색 페이지 및 매칭 API(`app/api/match/route.ts`) 수정 완료 (Postgrest undefined_column 오류 해결).
- **성향 분석 조건부 안내 유도**: 성향 분석을 아직 받지 않은 사장님도 공고 등록을 완전 자유롭게 허용하며, 상세 페이지 내에서만 가볍게 성향 분석을 유도하는 참여형 카드 형태로 개편.


**구현 완료 (2026-07-16)**

**순수 Canvas 기반 이미지 크롭 툴 전 플랫폼 통합 이식**
- **초경량 자체 크로퍼 개발** ([ImageCropModal.tsx](file:///c:/pazabv2/components/ImageCropModal.tsx)):
  - 외부 NPM 라이브러리 의존성 없이 React `useState`, `useRef` 및 HTML5 가상 `<canvas>`를 사용해 직접 조작 가능한 **초경량 크롭 모달** 구현.
  - 마우스 드래그 및 모바일 터치 스와이프를 모두 활용하여 이미지의 위치를 유연하게 배치할 수 있는 **Pan 기능**과 하단 범위 입력기(`type="range"`)를 사용하여 100% ~ 300%까지 세밀하게 조절하는 **Zoom 기능**을 제공합니다.
  - **`isCircle?: boolean` (원형 가이드라인) 속성**을 구현하여, 개인 프로필 아바타 자르기 시에는 동그란 가이드를 제공.
- **전 플랫폼 이미지 크롭(Crop) 경험 일괄 통일**:
  - **매장 등록/수정 모달** ([StoreRegisterModal.tsx](file:///c:/pazabv2/components/StoreRegisterModal.tsx)): 매장 로고(1:1 사각형 가이드) 및 매장 커버(16:9 와이드 가이드)에 알맞게 크로퍼가 조건부 실행되도록 연동 완료.
  - **마이페이지 개인 아바타 수정** ([page.tsx (mypage)](file:///c:/pazabv2/app/mypage/page.tsx)): 개인 프로필 사진 변경 시 원형 크롭 가이드를 띄우고 기존의 복잡했던 아바타 확인 모달을 완전히 제거하여 편집 동선 정교화.
  - **구직자 프로필 등록/수정** ([page.tsx (worker/profile)](file:///c:/pazabv2/app/worker/profile/page.tsx)): 구직 대표 사진 업로드 시 원형 크롭 가이드를 연동해 1:1 자르기 제공.
  - **공고 등록/수정** ([page.tsx (employer/register)](file:///c:/pazabv2/app/employer/register/page.tsx)): 공고용 대표 사진 업로드 시 16:9 와이드 크롭으로 완벽하게 통일.

**소식 상세 모달 둥글기 개선 및 풀스크린 줌 라이트박스 캐러셀 구현**
- **상세 모달 코너 라운딩 최적화** ([page.tsx (store/[id])](file:///c:/pazabv2/app/store/%5Bid%5D/page.tsx), [page.tsx (profile/[userId])](file:///c:/pazabv2/app/profile/%5BuserId%5D/page.tsx)):
  - 기존에 화면의 세로 94%를 꽉 메운 채로 너무 둥글게 깎여 둔탁하고 둔해 보이던 `rounded-3xl` 코너 라운딩을 전격 폐지하고, 현대적이고 샤프하며 세련된 도시형 피드 감각인 **`rounded-xl` (12px)**로 일괄 정밀 변경했습니다.
- **전면 풀스크린 줌 라이트박스 뷰어 캐러셀 구현**:
  - 상세 화면의 사진 영역을 탭하는 즉시 전면 딥 블랙(Black) 배경의 100% 가득 찬 풀스크린 줌 뷰어(`zoomedMedia`)가 띄워지도록 고도화.
  - 줌 뷰어 안에서도 좌우의 큰 화살표 버튼을 통해 여러 장의 사진들을 왜곡 없이(`object-contain`) 앞뒤로 시원시원하게 넘기며 감상할 수 있도록 슬라이더를 장착했습니다.

**상세 모달 투트랙 슬라이드 내비게이션(피드 이동 + 사진 캐러셀) 이식**
- **이전/다음 피드 이동 단추 추가**:
  - 상세 모달 카드 좌우 외벽선에 이전/다음 소식으로 바로 건너갈 수 있는 반투명 원형 화살표 버튼(`<`, `>`)을 배치하여 매번 모달을 닫고 다시 클릭해야 하던 탐색 피로도를 대폭 줄였습니다.
- **모달 내부 다중 이미지 캐러셀 분리 보완**:
  - 피드 이동 화살표(카드 바깥)와 글 내부의 다중 사진 이동 화살표 간의 기능 충돌을 해결하기 위해, 상세 모달 미디어 영역 내부 좌우 양끝에 **독립된 이미지 슬라이더용 미세 단추(`<`, `>`)**와 **인디케이터 배지**를 배치했습니다.
  - 포스트 자체는 카드 바깥 화살표로 빠르게 넘기고, 포스트 내부의 사진들은 이미지 위 화살표로 직관적으로 넘겨볼 수 있는 이중 내비게이션을 완성했습니다.

**매장 홈 상단 글로벌 메뉴 복원 및 다이렉트 소식 등록 흐름 연동**
- **AppHeader 글로벌 메뉴(더보기/알림종) 복원**:
  - 상세 정보 페이지의 뒤로가기(`showBack`)로 인해 자동 숨김 처리되던 알림종과 글로벌 세로 점 3개 메뉴를 원래 의도대로 활성화하기 위해 `AppHeader`에 **`showBellAndMenu={true}`** 속성을 부여하여 대타SOS/피드/스도쿠/설정이 정석적으로 동시 복원되었습니다.
- **내 채널 내 다이렉트 소식 등록 버튼 추가**:
  - 본인 소유의 매장 홈에 접속(`isOwner === true`)하거나 로그인한 본인의 프로필 페이지(`myId === profileId`)인 경우, 타이틀 옆에 **`[+ 소식 등록]`** 버튼이 생성되며 클릭 시 `/feed?write=true`로 즉시 연결됩니다.
- **피드 페이지 자동 모달 전개 및 뒤로가기 재오픈 버그 완치**:
  - `useSearchParams`를 연동하여 `write === "true"` 파라미터가 감지되면 페이지 진입 즉시 피드 글쓰기 모달을 자동 전개하고, `storeId` 인자가 넘어온 경우 해당 매장을 자동으로 기본 매핑해 줍니다.
  - 글 작성 창이 전개된 직후 Next.js `router.replace("/feed", { scroll: false })`를 신속하게 호출하여 주소창의 쿼리 스트링을 말끔히 소거함으로써, 작성 완료 혹은 취소 후 다른 곳을 탐색하다가 브라우저 **뒤로가기(Back)를 실행했을 때 모달이 다시 열리던 고질적인 탐색 루프 버그를 근본적으로 완치**했습니다.
  - Next.js 정적 프리렌더 단계에서 발생할 수 있는 빌드 에러(`missing-suspense-with-csr-bailout`)를 완차하기 위해, 피드 페이지 최하단 렌더러를 **`<Suspense>` 경계**로 안전하게 감싸서 배포 결함을 사전 방지했습니다.

