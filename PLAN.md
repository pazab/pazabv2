# PLAN.md
> 최종 업데이트: 2026-08-13 | PAZAB v2 (`C:\pazabv2`, Supabase: clrjxxkgceluvzvrkvyl)

## 구현 완료

**파잡 커리어 정리 + 매칭점수 전면 재설계 + 대타 노쇼 자동판정 신설 + 지원자 관리 신설 + MY 페이지 대타 동선 정리 (2026-08-13)**
> 상세 설계 배경은 [DESIGN_PLAN.md §11](DESIGN_PLAN.md)(정산·결제 인프라 설계, 노쇼 자동판정 타임라인) 참조.

- **파잡 커리어(`/worker/[id]`) 정리**: 히어로에 닉네임 누락 수정, 죽은 `TeamCompatSection`/`team-compat` API 제거, 이미 활성 팀원인 상대에게 "결렬됨·다시보내기"가 뜨던 버그 수정, 대타 SOS 거절 이력이 일반 채용매칭 상태에 새던 것 차단.
- **매칭점수("궁합") 개념 폐기**: `match_score` 단일 숫자 대신 `wage_ok`/`days_overlap` 개별 지표로 분리(`app/api/match/route.ts`), `getMatchLevel` 등 연애매칭식 라벨 전부 제거.
- **대타 "노쇼 안심 보장 수수료"(결제 목업) 폐기** → 무료 대안(신뢰 시그널 노출/페널티 명시/노쇼 시 즉시 자동 재확산)으로 대체. 가짜 1원인증 관련 배지도 정리.
- **대타 자동 노쇼 판정 타임라인 신설**: 알바생 원탭 체크인(`app/api/daeta/checkin`), 사장님 10분 연장(`app/api/daeta/extend`), +15분 미체크인 시 자동 노쇼+재확산(`app/api/cron/daeta-checkin`, 5분 주기 — **cron-job.org 등록 완료**). `lib/daetaNoShow.ts`로 수동/자동 노쇼 처리 로직 통합.
- **대타 이탈 처리 구멍 2건 수정**: 사전 취소가 노쇼보다 대우가 나빴던 버그(`daeta_postings` 재오픈 안 됨) 수정, 경합 지원자(같은 공고에 여러 명 지원 시) 방치되던 버그 수정 — `lib/daetaEscalation.ts`에 `reescalateAfterDropout()` 공용 함수 신설.
- **지원자 관리 기능 신설**: 그동안 `/daeta` 어디에도 지원자 목록/수락 UI가 없어서 MY페이지의 별개 "지원현황"까지 찾아가야 했던 문제 — "내 공고" 카드의 "응답 N건"을 눌러 지원자 목록(닉네임+신뢰등급+파잡커리어 링크+수락 버튼) 바텀시트로 확인 가능하도록 `DaetaSosHome.tsx`에 신설.
- **대타 이력 페이지 이원화 버그 수정**: `app/daeta/page.tsx`가 `roleView` 상태를 계산만 하고 `<DaetaSosHome>`에 전달을 안 해서 both계정 기준이 화면마다 어긋나던 버그 수정. `DaetaHistoryView`가 아직 결정 안 난 pending 지원자까지 "이력"에 섞어서 "완료된 기록 없음"처럼 보이던 것도 pending 제외로 수정 — 이제 "지원자"(진행중)와 "이력"(끝난 것)이 명확히 분리됨.
- **MY 페이지 "내 채용 활동" 잔재 정리**: 안 쓰는 일반 채용공고("매장 공고") 인라인 목록 UI + 죽은 상태(`jobs`/`toggleJobActive`/`deleteJob`) 전부 제거. `/daeta`가 이미 바텀네비 상시 탭이라 MY 안에 중복 링크를 만들 필요 없다는 걸 확인, 최종적으로 버튼 추가 없이 정리만 함.
- 대타 등록 화면(`DaetaRegisterModal`)의 자동 할증 상한 기본 선택값 30%→20% 완화(사장님 심리적 거부감 우려).
- 대타 공고 카드에 주소 텍스트 + 탭하면 지도(`map.html` 재사용) 뜨는 상세 바텀시트 신설. 매장 홈(`/store/[id]`)에도 동일하게 지도 추가.

**다음 세션 할 일**:
- [ ] 지원자 목록 바텀시트에 "수락"만 있고 "거절" 버튼이 없음 — 추가 필요 (지원자가 거절당한 걸 알 방법이 없음, 알림도 없음)
- [ ] 지원자 보기 팝업 UI 디자인 다듬기 (현재는 기능만 급하게 붙인 상태)
