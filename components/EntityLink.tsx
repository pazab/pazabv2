...
export function EntityLink({
  ...
  if (variant === "chip") {
    return (
      ...
      >
        <div style={{ width: 14, height: 14, borderRadius: "50%", background: avatarUrl ? `url(${avatarUrl}) center/cover` : fallbackGradient, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 7, overflow: "hidden" }}>
          {!avatarUrl && fallbackEmoji}
        </div>
        <span style={{ fontSize: 9, fontWeight: 700, color: "var(--primary, #8b5cf6)", letterSpacing: "0.2px" }}>{label} →</span>
      </span>
    );
  }
  ...
  return (
    ...
    >
      <div style={{ width: 24, height: 24, borderRadius: "50%", background: avatarUrl ? `url(${avatarUrl}) center/cover` : fallbackGradient, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, overflow: "hidden" }}>
        {!avatarUrl && fallbackEmoji}
      </div>
