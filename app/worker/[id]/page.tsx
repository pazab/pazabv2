      ...
      <div style={{ padding: "0 20px" }}>
        ...
        <div style={{ padding: "20px 0", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {[
              { icon: "₩", label: "희망 시급", value: Number(w.desired_wage) > 0 ? `${Number(w.desired_wage).toLocaleString()}원` : "협의", sub: w.wage_negotiable ? "협의 가능" : null, color: "#fca5a5" },
        ...
        {credentials.length > 0 && (
          <div style={{ padding: "20px 0", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {credentials.map((c) => (
                <span key={c.name} style={{ fontSize: 12, background: c.is_mandatory_by_law ? "rgba(236,72,153,0.1)" : "rgba(139,92,246,0.1)", border: `1px solid ${c.is_mandatory_by_law ? "rgba(236,72,153,0.25)" : "rgba(139,92,246,0.25)"}`, color: c.is_mandatory_by_law ? "#f9a8d4" : "#c4b5fd", padding: "6px 12px", borderRadius: 20 }}>
          ...
          <div style={{ padding: "20px 0", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {feeds.map((feed: Record<string, unknown>) => {
                ...
                return (
                  <div key={String(feed.id)} onClick={() => router.push("/feed")} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16, padding: 12, display: "flex", gap: 12, cursor: "pointer" }}>
                    {imgUrl && (
